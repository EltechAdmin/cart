...
function load_page_tpl ($page_id, $page_template)
{
	...
	else
	{
		...
		else
		{
			...
			if (!$tpl) $tpl = load_tpl ('page_default.tpl');
		}
	}
	return $tpl;
}
...
if ($isPermalink && !$permalink_param) $page_id = $original_idx;
elseif ($isPermalink && $permalink_param) $cid = $original_idx;

// mode
$cmd = 'default';
if (!empty ($page_id)) $cmd = 'pid';
if ($cid) $cmd = 'cid';
...
switch ($cmd)
{
	case 'download':
		...
		{
			if (!strpos ($row['page_download'], strval ($current_user_level))) just_die ($lang['msg']['no_level']);
		}
		if (empty ($row)) { $txt['main_body'] = '<h1>Page not found!</h1>'; flush_tpl (); }
		if (empty ($row['page_attachment'])) msg_die ($lang['msg']['page_attachment_error']);
		$fn = './public/file/'.$row['page_attachment'];
		if (!file_exists ($fn)) msg_die ($lang['msg']['page_attachment_error']);
	...
	case 'pid':
		...
		// is available?
		if (empty ($row)) { fullpage_die ($lang['l_page_not_found']); }

		// is draft?
		if ($row['page_status'] == 'D' && !$current_admin_level) { fullpage_die ($lang['l_page_not_found']); }

		// is member only?
		if (($row['page_status'] == 'M') && !$isLogin) { fullpage_die ($lang['l_page_member_only']); }

		// is admin only?
		if (($row['page_status'] == 'A') && !$current_admin_level) { fullpage_die ($lang['l_page_admin_only']); }
		...
		if (!$content)
		{
			...
			// page template
			if (empty ($row['page_template']) && empty ($rules['page_template'])) $page_template = 'page_default.tpl';
			elseif (empty ($row['page_template']) && !empty ($rules['page_template'])) $page_template = $rules['page_template'];
			else $page_template = $row['page_template'];
			if ($page_template == '_blank') $blank_tpl = true; else $blank_tpl = false;

			if (!$rules['page_comment'] && $row['page_allow_comment']) $allow_comment = false;
			if ($allow_comment && $enable_facebook_comment)
			{
				$allow_comment = false;
				$enable_page_facebook_comment = true;
			} else $enable_page_facebook_comment = false;
			...
			if ($row['page_mode'] == 'raw') $row['page_body'] = quick_tpl ($row['page_body'], array ());

			// main_image
			$row['page_image_th'] = '';
			if (!$main_image) $main_image = $main_image_th = false;
			if ($main_image && !$row['page_image']) $main_image = $main_image_th = false;
			if ($main_image && $main_image_th && $row['page_image']) { $row['page_image_th'] = $row['page_image']; $main_image = false; $main_image_thumb = true; }
			...
			if ($row['page_related'] && !$blank_tpl)
			{
				...
				{
					if ($config['enable_adp'] && $row2['permalink']) $row2['page_url'] = $row2['permalink']; else $row2['page_url'] = "page.php?pid=$row2[page_id]";
				...
				if (!empty ($row['block_list'])) $sub_page = true;
			}
		...
		if ($current_admin_level) $txt['main_body'] = sprintf ($lang['edit_in_acp'], $page_id).$txt['main_body'];
		...
		if ($row['page_status'] == 'D' && $current_admin_level) { $txt['main_body'] = $lang['l_page_draft'].$txt['main_body']; flush_tpl (); }
	...
	case 'cid':
		...
		// is listing allowed?
		if (!$rules['cat_list']) msg_die ($lang['msg']['no_cat_list']);
		if ($rules['all_cat_list']) $all_cat_list = true; else $all_cat_list = false;
		...
		if ($all_cat_list)
		{
			...
			{
				$i++;
				if ($config['enable_adp'] && $row['permalink']) $row['cat_url'] = $row['permalink']; else $row['cat_url'] = "page.php?cid=$row[idx]";
		...
		if (!$i) ($all_cat_list = false);

		// page list
		if (empty ($sort)) $sort = $rules['page_sort'];
		if ($sort == 'd') $s = 'page_id DESC';
		else { $s = 'page_title'; $sort = 't'; }
		...
		foreach ($foo as $k => $v)
		{
			if ($config['enable_adp'] && $v['permalink']) $v['page_url'] = $v['permalink']; else $v['page_url'] = "page.php?pid=$v[page_id]";
			if (!empty ($v['page_attachment'])) $v['page_attachment'] = $lang['page_attachment']; else $v['page_attachment'] = '';
			if ($v['page_pinned']) $v['page_pinned'] = $lang['page_pinned']; else $v['page_pinned'] = '';
		...
		if ($current_admin_level) $txt['main_body'] = sprintf ($lang['edit_pcat_in_acp'], $cid).$txt['main_body'];
