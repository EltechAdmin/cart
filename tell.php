...
switch ($cmd)
{
	case 'send':
		...
		if (!validate_email_address ($txt['email'])) msg_die ($lang['msg']['tell_error']);

		// get product info
		if ($item_id) $txt = array_merge ($txt, $item);
		...
		if (($who == 'friend') && !empty ($item['title']))
		{
			...
			if (!validate_email_address ($txt['friend_email'])) msg_die ($lang['msg']['tell_error']);
	...
	default:
		...
		if ($item_id)
		{
			...
			foreach ($item['bread_item'] as $k => $v) $item['block_cat_bread_crumb'] .= quick_tpl ($tpl_block['cat_bread_crumb'], $v);
