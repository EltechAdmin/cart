...
switch ($cmd)
{
	case 'add':
		foreach ($_POST['item_id'] as $key => $val)
		{
			...
			if ($qty && $item_id)
			{
				...
				if ($prod['digital_file'] && !$isLogin) msg_die ($lang['l_digital_not_login']);
				if ($prod['is_call_for_price']) msg_die ($lang['l_cart_call_price']);
				if ($prod['stock'] < 1) msg_die ($lang['msg']['no_stock']);
				...
				if ($qty < $min_buy) msg_die ($lang['l_min_max_buy_err']);
				if ($qty > $max_buy) msg_die ($lang['l_min_max_buy_err']);
				...
				else
				{
					...
					if ($avail < $row['qty'] + $qty) $qty = $avail - $row['qty'];
					if ($row['qty'] + $qty > $max_buy) $qty = $max_buy - $row['qty'];
	...
	case 'update':
		if (empty ($_GET['qty'])) redir ();

		foreach ($_GET['qty'] as $key => $val) if (is_numeric ($key) && is_numeric ($val))
		{
			if ($val <= 0)
			...
			else
			{
				...
				if ($val > $avail) $qty = $avail; else $qty = $val;
