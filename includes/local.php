...
function make_thumb ($image_id, $mode, $admin = false)
{
	...
	if ($in_admin_cp) $admin = true; else $admin = false;
	...
	{
		if (($mode == 'feature') || ($mode == 'gallery')) $mode = 'detail';
	...
	else
	{
		...
		if ($mode == 'small') { $img_th = $img_sm; $size = 50; } else $size = 'thumb';
...
function verify_selected ($selected, $options)
{
	...
	if (!is_array ($selected)) { $selected = array ($selected); $is_arr = false; }
	...
	{
		$vv = str_replace ('=', '%3D', $v);
		if (array_key_exists ($vv, $foo)) $verified[] = $foo[$vv];
	}
...
function get_custom_field ($cf_arr, $is_detail = true)
{
	...
	foreach ($ce_cache['cf_define'] as $row)
	{
		...
		{
			case 'file':
				if (!empty ($val) && $is_detail) $custom = "<a href=\"$ffolder/$val\">$val</a>";
			break;
			...
			case 'rating':
				if (!empty ($val))
				{
					...
					if ($row['is_searchable']) $custom = "<a href=\"$site_url/shop_search.php?$cf_search\">$custom</a>";
				}
			break;
			...
			case 'radio':
				if ($val)
				{
					...
					$custom = $val;
					if ($row['is_searchable']) $custom = "<a href=\"$site_url/shop_search.php?$cid=$fii\">$custom</a>";
				}
			break;
			...
			default:
				...
				{
					$custom = $val;
					if ($row['is_searchable']) $custom = "<a href=\"$site_url/shop_search.php?cfonly=1&amp;$cf_search\">$val</a>";
				}
			break;
		}
		...
		{
			$i++;
			if ($row['is_list'] || $is_detail) $output[$cid] = array ('cf_idx' => $cid, 'cf_title' => $row['cf_title'], 'cf_value' => $custom, 'cf_type' => $row['cf_type'], 'cf_raw' => $val);
		}
	}
...
function process_product_info (&$row, $mode = 'list')
{
	...
	if ($row['digital_file']) $row['digital'] = $lang['l_digital_icon_small']; else $row['digital'] = '';
	if ($row['is_call_for_price']) $row['price'] = $lang['l_list_call_price'];
	...
	foreach ($foo as $k => $v) $breadcat[] = array ('bc_link' => $ce_cache['cat_url'][$v], 'bc_title' => $ce_cache['cat_name_def'][$v]);
	$breaditem = $breadcat;
...
function send_digital_products ($order_id, $auto_complete = false)
{
	...
	$prod_list = array (); $all_digital = true;
	...
	while ($row = sql_fetch_array ($res))
	{
		if ($row['digital_file'])
		{
			...
			if (!$foo) sql_query ("INSERT INTO ".$db_prefix."user_file SET user_id='$summary[user_id]', item_id='$row[item_id]', order_idx='$order_id'");
		}
		else
			$all_digital = false;
	}
	if ($all_digital) $result['all_digital'] = true;
	...
	if ($auto_complete && $result['any_digital'])
	{
		...
		$summary['block_list'] = '';
		foreach ($prod_list as $k => $v) $summary['block_list'] .= quick_tpl ($tpl_block['list'], $v);
	...
	if ($result['any_digital']) return $result; else return false;
}
...
function get_cat_structure ($cat_id = 0, $level = 0, $prefix = '', $prefix_id = '')
{
	...
	if (!isset ($ce_cache['cat_structure_html'])) $ce_cache['cat_structure_html'] = '';
	...
	while ($row = sql_fetch_array ($res))
	{
		...
		if (!$level) $ce_cache['cat_structure_top'][$row['idx']] = $row['cat_name'];
		if ($config['enable_adp'] && $row['permalink'])
	...
	if ($exists) { $ce_cache['cat_structure_html'] .= str_repeat ("\t", $level)."</ul>\n"; }
}
...
function create_search_cache ($item_id)
{
	...
	{
		if (in_array ($v['cf_type'], $cfsc) && ($v['is_searchable'])) $cfsc_str .= $row[$k].' ';
	}
...
function delete_item ($item_id)
{
	...
	while (!$ok)
	{
		$i++;
		$fn = $item_id.'_'.$i.'.jpg';
		if (!@unlink ("$config[abs_path]/public/products/$fn")) $ok = TRUE;
		if (!@unlink ("$config[abs_path]/public/products_thumbs/$fn")) $ok = TRUE;
	}
}
...
{
	if (!is_numeric($val) || (($val < 0) || ($val > 5))) return false;
	if (!$val) return false;
	$s = $val - 0.5;
	$e = $val + 0.49;
	if ($s < 0.51) $s = 0.1;
...
function local_init ()
{
	...
	$ok = true;
	foreach ($cat_cache_def as $k => $v) if (empty ($foo[$v])) $ok = false;
	if (!$ok)
	{
		// cat sorting
		...
		while ($row = sql_fetch_array ($res)) $cat_sort[] = $row['idx'];
		$cat_sort = implode (',', $cat_sort);
		if (empty ($cat_sort)) $cat_sort = 0;
		...
		while ($row = sql_fetch_array ($res)) $cf_custom_sort[] = $row['idx'];
		...
		foreach ($cat_cache_def as $k => $v) { qcache_update ($v, serialize ($ce_cache[$v])); }
	}
	else
		foreach ($foo as $k => $v) { $ce_cache[$k] = unserialize ($foo[$k]); }
	$ce_cache['ok'] = true;
}
