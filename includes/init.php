...
// init database
if (!$dbh = mysqli_connect ($db_hostname, $db_username, $db_password)) { echo mysqli_error (); exit; }
...
if (!$res) die ("<h1>Fatal Error!</h1><p>Can not connect to configuration table. Please verify your database configuration.</p><p><b>MySQL Respond:</b> ".mysqli_error ($dbh)."</p>");
...
if (isset ($_SESSION[$db_prefix.'override_skin'])) $config['skin'] = 'skins/'.$_SESSION[$db_prefix.'override_skin'];
if (isset ($_SESSION[$db_prefix.'language'])) $config['default_lang'] = $_SESSION[$db_prefix.'language'];
...
// debug info
if (function_exists ('memory_get_usage')) $memory_when_start = memory_get_usage (); else $memory_when_start = 0;
...
if ($config['enable_module_engine'])
{
	...
	while ($row = sql_fetch_array ($res))
	{
		...
		$css = explode ("\n", $row['mod_css']);
		$js = explode ("\n", $row['mod_js']);
		foreach ($css as $k => $v) if (!empty ($v)) $module_css_list[] = "<link rel=\"stylesheet\" type=\"text/css\" href=\"$config[site_url]/skins/_module/$v\" />";
		foreach ($js as $k => $v) if (!empty ($v)) $module_js_list[] = "<script type=\"text/javascript\" src=\"$config[site_url]/skins/_module/$v\"></script>";
		if (!empty ($module_css_list)) $txt['module_css_list'] .= implode ("\n", $module_css_list)."\n";
		if (!empty ($module_js_list)) $txt['module_js_list'] .= implode ("\n", $module_js_list)."\n";
	}
}
	...
	if (!$foo['lang_value'])
	{
		...
		while ($row = sql_fetch_array ($res))
		{
			if ($row['lang_key'][0] != '_')
			{
				...
				if (!empty ($f[2])) $lang[$f[0]][$f[1]][$f[2]] = str_replace ('__SITE__', $config['site_url'], $row['lang_value']);
				elseif (!empty ($f[1])) $lang[$f[0]][$f[1]] = str_replace ('__SITE__', $config['site_url'], $row['lang_value']);
				else $lang[$row['lang_key']] = str_replace ('__SITE__', $config['site_url'], $row['lang_value']);
			}
		}
