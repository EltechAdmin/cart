...
function get_courier_fee ($summary, $user, $method = '')
{
	...
	if ($method)
	{
		if ($x = strpos ($method, '.')) $mod = substr ($method, 0, $x); else $mod = $method;
		$row_mod = sql_qquery ("SELECT * FROM ".$db_prefix."module WHERE mod_enabled = '1' AND mod_type = 'shipping' AND mod_id='$mod' LIMIT 1");
		if (!file_exists ('./module/'.$mod.'/window.php')) msg_die (sprintf ($lang['msg']['internal_error'], 'Shipping module '.$method.' not found.'));
	...
	else
	{
		...
		while ($row_mod = sql_fetch_array ($res_mod))
		{
			...
			$ship_info = array ();
			if (!file_exists ('./module/'.$method.'/window.php')) msg_die (sprintf ($lang['msg']['internal_error'], 'Shipping module '.$method.' not found.'));
			require ('./module/'.$method.'/window.php');
			if (!empty ($ship_info))
			{
				if (!empty ($ship_info[0]))
				{
					foreach ($ship_info as $k => $v) $tmp[$v['method']] = $v;
				}
				else
...
function get_payment_method ($method = '')
{
	...
	if (!$method)
	{
		...
		while ($row = sql_fetch_array ($res))
		{
			...
			$payment_extra_fee = 0;
			$mod = $row['mod_id'];
			if (!file_exists ('./module/'.$mod.'/window.php')) msg_die (sprintf ($lang['msg']['internal_error'], 'Payment module '.$method.' not found.'));
			require ('./module/'.$mod.'/window.php');
...
function get_payment_form ($method, $user, $summary)
{
	...
	// load related module
	$payment_cmd = 'form';
	if (!file_exists ('./module/'.$method.'/window.php')) msg_die (sprintf ($lang['msg']['internal_error'], 'Payment module '.$method.' not found.'));
	require ('./module/'.$method.'/window.php');
	return $form;
}
...
function update_stock ($idx, $stock, $qty, $digital)
{
	...
	if (!$config['cart']['manage_stock']) return true;
...
function get_cart ($tpl, $tpl_adm = '', $update_stock = false, $order_id = '')
{
	...
	$all_digital = true; $display = $display_adm = '';
	...
	while ($row = sql_fetch_array ($res))
	{
		...
		if ($stock_avail)
		{
			...
			if ($pq) foreach ($pq as $k => $v) if ($row['qty'] >= $k) $row['price'] = $v;
			...
			if ($tax) $taxable = $taxable + $subtotal;
			if (empty ($row['digital_file'])) $all_digital = false;
			...
			if ($tpl_adm) $display_adm .= quick_tpl ($tpl_adm, $row);
