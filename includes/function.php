...
function msg_die ($msg_txt, $url = '')
{
	...
	if ($msg_txt == 'ok') $msg_txt = $lang['msg']['ok'];
...
function num_format ($number, $comma = 0, $currency = 0, $zeros = 0)
{
	...
	if ($currency)
	{
		...
		if ($zeros && (empty ($number) || $number == 0)) return $zeros;
		if (!$comma) $comma = $config['num_decimals'];
		$val = number_format (abs ($number), $comma, $config['num_dec_point'], $config['num_thousands_sep']);
		if ($currency == 1) if ($config['num_curr_pos']) $val .= $config['num_currency']; else $val = $config['num_currency'].$val;
		if ($neg) $val = '-'.$val;
	}
	else
	{
		if ($zeros && empty ($number)) return $zeros;
...
function array_clean ($array, $reset_key = false)
{
	...
	{
		if (is_array ($array[$index])) { $array[$index] = array_clean ($array[$index]); $multi_array = true; }
		if (empty($value)) unset($array[$index]);
	}
	if ($reset_key && $multi_array) return array_map ('array_values', $array);
	elseif ($reset_key && !$multi_array) return array_values ($array);
	else return $array;
}
...
function cur_url ($encode = true)
{
	...
	if (!$isPermalink) $isPermalink = get_param ('permalink_request') ? true : false;

	$script = '';
	if ($isPermalink || !empty ($g)) $script = get_param ('permalink_request');								// for permalink, get url from permalink_request var, otherwise the url is always permalink.php
	if (empty ($script) && !empty ($_SERVER['PHP_SELF'])) $script = $_SERVER['PHP_SELF'];					// should be working..
	...
	if (empty ($script)) die ('Can not determine current url! Contact us now!');							// what the f__?!
...
function check_mobile ()
{
	...
	global $config;
	if (!$config['mobile_version']) return false;
	$isMobile = false;
	...
	if (strpos($ac, 'application/vnd.wap.xhtml+xml') !== false || $op != '') $isMobile = TRUE;
	foreach ($mobile_def as $val) if (strpos ($ua, $val) !== false) $isMobile = TRUE;
...
function sql_query ($sql, $debug = false)
{
	...
	if ($debug && $config['debug_mode']) echo $sql.'<br />';
	$debug_info['sql'][] = nl2br (chunk_split ($sql, 120));
	if (!$result = mysqli_query ($dbh, $sql))
	{
		if ($config['debug_mode']) { echo '<h1>MySQL Error</h1>'.$sql; getCallingFunction (); }
...
function sql_fetch_assoc ($res_id, $allow_html = 1, $slash = 1)
{
	...
	foreach ($foo as $k => $v) if (is_int ($k)) unset ($foo[$k]);
	return $foo;
}
...
function create_blank_tbl ($tbl)
{
	...
	while ($foo = sql_fetch_array ($res)) $row[$foo['Field']] = '';
	return $row;
}
...
function sql_multipage ($table, $columns, $where, $order_by, $cur_page, $script_name = '', $per_page = '', $debug = false)
{
	...
	if ($debug && $config['debug_mode']) echo $sql;
	$res = sql_query ($sql);
...
function generate_pagination ($script_name, $total_pages, $cur_page, $num_item)
{
	...
	for ($i = $start; $i <= $finish; $i++)
	{
		if ($i == $cur_page)
		{
			$page_string[] = "<a href=\"#\">$i</a>"; $class_string[] = 'active';
		}
		else
		{
			$page_string[] = "<a href=\"$base_url&amp;p=$i\">$i</a>"; $class_string[] = 'normal';
		}
	}
	...
	{
		$row['pg_prev'] = "<a href=\"#\">".$lang['l_pp_prev']."</a>"; $row['pg_prev_class'] = 'active';
	}
	...
	{
		$row['pg_next'] = "<a href=\"#\">".$lang['l_pp_next']."</a>"; $row['pg_next_class'] = 'active';
	}

	// first & last pages
	if ($cur_page == 1)
	{
		$row['pg_top'] = "<a href=\"#\">".$lang['l_pp_top']."</a>"; $row['pg_top_class'] = 'active';
	}
	else
	...
	if ($cur_page == $total_pages)
	{
		$row['pg_last'] = "<a href=\"#\">".$lang['l_pp_last']."</a>"; $row['pg_last_class'] = 'active';
	}
	else
...
function create_page_status_sql ()
{
	...
	if (!$isLogin && !$config['allow_locked_page_list']) $foo[] = "page_status != 'M'";
	if (!$current_admin_level) $foo[] = "page_status != 'A'";
	$foo[] = "page_status != 'H'";
...
function sql_copy_row ($table_src, $key, $idx, $table_tgt = '')
{
	...
	if (!$table_tgt) $table_tgt = $table_src;
	...
	if (session_param ($db_prefix.'isLogin'))
	{
		...
		if (session_param ($db_prefix.'auth') == $check)
		{
			...
			if (!empty($row['user_id']))
			{
				...
				foreach ($row as $k => $v) if (!is_numeric ($k)) $current_user_info[$k] = $v;
...
function create_select_form ($select_name, $source, $selected_value = '', $first_line = '', $disabled = 0, $addtl_option = '')
{
	...
	if (!empty ($source)) foreach ($source as $key=>$val)
	{
		$foo = '';
		$doh = substr ($key, 0, 3);
		...
		else
		{
			...
			elseif ($key == $selected_value) $foo .=  " selected=\"selected\"";
			if (substr ($val, 0, 2) == '//')
...
function time_form ($prefix, $select = '', $interval = 5, $disabled = false)
{
	...
	if (empty ($select) && !verify_time ($select)) $select = '00:00';
	...
	for ($h = 0; $h <= 23; $h++)
	{
		if (strlen ($h) <= 1) $hi = '0'.$h; else $hi = $h;
		$hh[$hi] = $h;
	}
	...
	{
		if (strlen ($m) <= 1) $mi = '0'.$m; else $mi = $m;
		$mm[$mi] = $mi;
	}
...
function date_param ($prefix, $method = 'get')
{
	...
	if (($yy < 1901) || ($yy > 2038)) return false;
...
function generate_html_header ($title = '', $description = '', $keywords = '')
{
	...
	if (empty ($title)) $title = $config['site_name'];
	if (empty ($description)) $description = $config['site_description'];
	if (empty ($keywords)) $keywords = $config['site_keywords'];
...
function rating_img ($rating, $fontsize = 20)
{
	...
	// full star
	for ($i=1; $i<=$s_rating; $i++) $img .= '<i class="glyphicon glyphicon-star" style="font-size:'.$fontsize.'px"></i>';

	// half star
	if ($s_rating < $rating)
	{
		$h_rating = $rating * 10;
		if (($h_rating / 5) % 2) { $img .= '<i class="glyphicon glyphicon-star half" style="font-size:'.$fontsize.'px"></i>'; $s_rating++; }
	}

	// empty star
	for ($i=$s_rating+1; $i<=5;$i++) $img .= '<i class="glyphicon glyphicon-star empty" style="font-size:'.$fontsize.'px"></i>';
	return $img.'</span>';
}
...
function redir ($url = '', $mode = 0)
{
	...
	if (empty ($url))
	{
		...
		if (strpos ('.'.$url, $config['site_url']) != 1) $url = $config['site_url'];
	}
...
{
	if (empty ($err_msg) && empty ($result)) { $result = $err_no; $err_no = 0; $err_msg = ''; }
...
function upload_file ($field, $target, $allow_overwrite = FALSE)
{
	...
	else
	{
		...
		if (!is_array ($target)) { $foo = array (); foreach ($field as $k => $v) $foo[$k] = $target; $target = $foo; }
		$array = TRUE;
	}
	...
	foreach ($field as $k => $v)
	{
		...
		if (!empty ($_FILES[$v]['tmp_name']) && !$_FILES[$v]['error'])
		{
			...
			foreach ($allowed as $dv)
			{
				...
				if (!in_array ($ext, $allowed)) $err = TRUE;
			}
...
function remote_fopen ($site_url)
{
	...
	if (ini_get ('allow_url_fopen')) return file_get_contents ($site_url, false, stream_context_create ($arrContextOptions));
...
function smtp_email ($to, $subject, $body, $html = 0)
{
	...
	$tom = $to;
	if ($x = strpos ($to, '<')) $to = substr ($to, $x + 1, -1);
	if ($config['debug_mode']) echo '<h1>Debug Result for SMTP Email</h1>';
	...
	// if ssl
	if ($config['smtp_secure'] == 'ssl') $SmtpServer = 'ssl://'.$SmtpServer;
	...
	// sender
	if (empty ($config['smtp_sender'])) $config['smtp_sender'] = $config['site_email'];
...
function email ($to, $subject, $body, $html = 0, $log = 0, $debug = 0)
{
	...
	// attachment?
	if (is_array ($body)) $att = true; else $att = false;
	$uid = md5 (uniqid (time()));
	...
	if ($debug && $config['debug_mode'])
	{
		...
		if ($config['debug_mode']) echo '<h1>Debug Mode</h1><p>Cookie may not be stored.</p>';
...
function code_editor_area ($id, $text = '', $lang = 'html', $width = 400, $height = 350, $is_editable = true)
{
	...
	if ($config['wysiwyg'])
	{
		...
		if ($config['multi_code_editor']) $code_editor_mode = 'code_editor_multi'; else $config['multi_code_editor'] = 1;
	}
	else
	{
	...
	$rte['site_url'] = $config['site_url'];
	// print_R ($rte);
	return quick_tpl (load_tpl ('editor.tpl'), $rte); die;
}
...
function image_watermark ($input, $watermark = '', $position = '', $output = '')
{
	...
	// default values
	if (!$watermark) $watermark = $config['watermark_file'];
	if (!$position) $position = $config['watermark_position'];
	...
	if (($format < 1) || ($format > 3)) return false; // unsupported image type
...
function qhash ($string, $mode = 'sha512', $advanced = true)
{
	...
	if ($advanced) $str = ~$string; else $str = $string;
	if ($mode == 'md5')
		return trim (md5 ($str));
	if ($mode == 'sha1')
...
function qcache_get ($id, $include_skin = true)
{
	...
	{
		$cid = '';
		foreach ($id as $v) $cid .= $include_skin ? "'$v:$config[skin]'," : "'$v',";
		$cid = substr ($cid, 0, -1);
		$num = count ($id);
	}
	else
	...
	{
		$output = array (); $num = 0;
		foreach ($id as $v) { $num++; $output[$v] = false; }
		if ($num > 1) return $output; else return false;
	}

	// get the cache
	$cached = array ();
	$res = sql_query ("SELECT * FROM ".$db_prefix."cache WHERE cache_id IN ($cid) LIMIT $num");
	while ($row = sql_fetch_array ($res)) $cached[$row['cache_id']] = $row['cache_value'];

	// verify the cache result
	foreach ($id as $v)
	{
		$cid = $include_skin ? "$v:$config[skin]" : $v;
		if (empty ($cached[$cid])) $cached[$cid] = '0|foo';	// if cache is empty, return empty cache (aka false or failed)
		$foo = explode ('|', $cached[$cid]);
		$lu = $foo[0];	// first section = time in unix stamp
		$cc = $foo[1];	// second section = the real cache contents
		if (time () - $lu > $config['cache']) $output[$v] = FALSE; else $output[$v] = (!empty ($cc)) ? gzuncompress (base64_decode ($cc)) : $cc;
	}

	if ($num > 1) return $output; else return $output[$oid];
}
...
function qcache_update ($id, $content, $include_skin = true, $cache_manual = false)
{
	...
	if (!mysqli_affected_rows ($dbh)) sql_query ("INSERT IGNORE INTO ".$db_prefix."cache SET cache_id='$id', cache_value='$cc', cache_manual='$cm'");
}
...
function xmlize($data, $WHITE=1)
{
	...
	if (!xml_parse_into_struct($parser, $data, $vals, $index)) sprintf("XML error: %s at line %d", xml_error_string(xml_get_error_code($parser)), xml_get_current_line_number($parser));
...
function read_xml ($filename, $debug = false, $show_warning = true)
{
	...
	{
		if (!file_exists ($filename)) { if ($show_warning) echo ('<p><b>Warning</b> '.$filename.' not found!</p>'); return false; }
		$tmp = @file_get_contents ($filename);
	}

	if (empty ($tmp)) { if ($show_warning) echo ('<p><b>Warning</b> '.$filename.' is empty or not found!</p>'); return false; }
	$xml = xmlize ($tmp);
...
function safe_send ($string, $trim = false)
{
	...
	// trim?
	if ($trim) foreach ($string as $k => $v) $string[$k] = trim ($v);
	foreach ($string as $k => $v) $string[$k] = urlencode (base64_encode ($v));
...
function format_address ($addr = '')
{
	...
	if (is_array ($addr))
	{
		if ($addr['address2']) $addr['address'] = $addr['address'].'<br />'.$addr['address2'];
	...
	{
		$foo = $config;
		if ($foo['site_address2']) $foo['site_address'] = $foo['site_address'].'<br />'.$foo['site_address2'];
...
function getCallingFunction ($summary_only = false)
{
	...
	if (!$summary_only)
	{
		...
		foreach ($trace as $k => $v) echo '<p>File <b>'.$v['file'].'</b><br />Function <b>'.$v['function'].'</b>:'.$v['line'].'</b></p>';
	}
	else
	{
...
function get_country_list ($cid = '')
{
	...
	if (!count ($country_list_def))
	{
		...
		foreach ($cfoo as $k => $v) $country_list_def[$v] = $v;
	}
