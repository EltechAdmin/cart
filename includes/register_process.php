...
if ($xpress)
{
	if (!$config['cart']['allow_xpress']) msg_die ('Express checkout is disabled!');
...
if ($cmd == 'address')
{
	if (!$isLogin) redir ($config['site_url'].'/profile.php');
}
else
{
	// visual confirmation
	...
	if (!empty($row['user_id'])) $err[] = "$lang[l_username_used]";

	// get email in db
	$row = sql_qquery("SELECT user_email FROM ".$db_prefix."user WHERE user_email='$user_email' LIMIT 1");
	if (!empty($row['user_email'])) $err[] = "$lang[l_email_used]";

	// validate entries
	if (!preg_match ("/^[[:alnum:]]+$/", $user_id) && !$xpress) $err[] = "$lang[l_username_error]";
	if (!validate_email_address($user_email)) $err[] = "$lang[l_email_error]";
	if (empty ($user_passwd)) $err[] = "$lang[l_password_empty]";
}
...
if (empty ($bill_zip)) $err[] = "$lang[l_zip_empty]";
if (empty ($ship_address)) $err[] = "$lang[l_address_empty]";
...
reset_form ();
if (!$xpress) $user_passwd = qhash ($user_passwd);
...
else
{
	...
	// xpress?
	if ($xpress) redir ($config['site_url'].'/checkout.php?step=2');
