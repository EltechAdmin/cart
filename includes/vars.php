...
if (empty ($current_user_info)) $current_user_info['user_level'] = $current_user_info['admin_level'] = 0;
if ($current_user_info['user_level'] > 5) $current_user_info['user_level'] = 5;
if ($current_user_info['admin_level'] > 5) $current_user_info['admin_level'] = 5;
if ($current_user_info['user_level'] < 1) $current_user_info['user_level'] = 0;
if ($current_user_info['admin_level'] < 1) $current_user_info['user_level'] = 0;
...
if (empty ($base_url)) $txt['request_location'] = 'index.php'; else $txt['request_location'] = html_unentities ($base_url);
if (strpos ($base_url, '/admin')) $txt['request_location'] = str_replace ($config['site_url'].'/'.$qe_admin_folder.'/', '', $base_url);
