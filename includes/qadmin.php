...
function qadmin_process ($def, $cfg)
{
	...
	// AXSRF
	if (empty ($cfg['admin_level'])) die ('<b>Fatal Error!</b> Admin level is not defined!');
	if ($cfg['admin_level']) AXSRF_check ();
	...
	if (!empty ($cfg['captcha']))
	{
		...
		if ($visual != qvc_value()) admin_die ($lang['msg']['captcha_error']);
	}
	...
	foreach ($def as $key=>$val)
	{
		...
		switch ($val['type'])
		{
			...
			case 'email':
				...
				if (!empty ($val['unique']))
				{
					...
					if (!empty ($foo)) $err[] = $val['title'].' must be unique!';
				}
				if (isset ($t)) $et = $tt = $t;	// $tt = for sql queries, $et = for email report
			break;


			case 'permalink':
				if (empty ($cfg['permalink_source']) || empty ($cfg['permalink_script'])) admin_die ($lang['msg']['permalink_error']);
				if (empty ($cfg['permalink_param'])) $cfg['permalink_param'] = '';
				if (empty ($cfg['permalink_folder'])) $cfg['permalink_folder'] = '';
				$t = post_param ($val['field']);
				if (empty ($t)) { $t = post_param ($cfg['permalink_source']); $auto = true; } else $auto = false;
				...
				if (!$permalink) $err[] = $val['title'].' must be unique!';
				if (isset ($t)) $et = $tt = $permalink;	// $tt = for sql queries, $et = for email report
			...
			case 'radiov':
				$val['edit_opt'] = '';
				if (!empty ($val['editopt'])) $val['option'] = get_editable_option ($val['editopt']);
			...
			case 'image':
				if (!empty ($_FILES[$val['field']]['name']) && (!$config['demo_mode']))
				{
					...
					if (!empty ($val['rename'])) $rnd = TRUE; else $rnd = FALSE;
					if ($val['type'] == 'file')
					{
					...
					if (!$x['success']) admin_die ($lang['msg']['can_not_upload']);
					...
					{
						if (!empty ($config['watermark_file'])) image_watermark ($fl, './../public/image/'.$config['watermark_file']);
			...
			case 'thumb':
				if (!empty ($_FILES[$val['field']]['name']) && !$config['demo_mode'])
				{
					$fn = $_FILES[$val['field']]['name'];
					$fm = $_FILES[$val['field']]['tmp_name'];
					if (!empty ($val['rename'])) $fn = get_filename ($cfg['table'], $val['field'], $fn);
			...
			case 'img_resize':
				if (!empty ($_FILES[$val['field']]['name']))
				{
					...
					$fm = $_FILES[$val['field']]['tmp_name'];
					if (!empty ($val['rename'])) $fn = get_filename ($cfg['table'], $val['field'], $fn);
					if (empty ($val['size'])) $val['size'] = $config['thumb_size'];
					if (!empty ($config['watermark_file'])) image_watermark ($fl, './../public/image/'.$config['watermark_file']);
			...
			case 'img_series':
				if (!empty ($_FILES[$val['field']]['tmp_name']) && !$config['demo_mode'])
				{
					...
					while (!$ok)
					{
						...
						if (!file_exists ("$cfg[img_folder]/$fn.jpg")) $ok = TRUE;
					}
		...
		// required?
		if (!empty ($val['required']) && empty ($tt)) $err[] = $val['title'].' is required!';

		// sql query (only do !empty field)
		if (isset ($tt)) $sql[] = "$val[field] = '$tt'";
		if (isset ($et)) $eml[] = "<tr><td valign=\"top\" class=\"form_title\"><b>$val[title]</b></td><td class=\"form_value\">$et</td></tr>";

		// fast search query
		if (!empty ($cfg['fastsearch']) && !empty ($val['index'])) $fs_query[] = $tt;
	}
	...
	if ($cfg['cmd'] == 'update')
	{
		...
		// update db
		if (!empty ($sql)) sql_query ("UPDATE $cfg[table] SET $sql WHERE $cfg[primary_key]='$primary_val' LIMIT 1");
		$id = $primary_val;
		...
		{
			if (empty ($cfg['detailed_log'])) { $foo = $old[$cfg['log_title']]; $old = $new = ''; }
			qadmin_log ($primary_val, $old[$cfg['log_title']], LOG_EDIT, $old, $new, $cfg['table']);
		}
	}
	elseif ($cfg['cmd'] == 'new')
	{
		if (!empty ($sql)) sql_query ("INSERT INTO $cfg[table] SET $sql");
		...
		$new = array ();
		if (!empty ($cfg['enable_log'])) qadmin_log ($id, $old[$cfg['log_title']], LOG_ADD, $old, $new, $cfg['table']);

		// if $id = 0, it means the primary key is not auto-increment, is it... varchar?
		if (empty ($id)) $id = post_param ($cfg['primary_key']);

		// send email?
		if (!empty ($cfg['send_to']))
		{
			global $title, $tpl_section;
			if (empty ($cfg['send_subject'])) $cfg['send_subject'] = 'Form Result';
	...
	// permalink
	if ($permalink) $permalink = generate_permalink ($permalink_cfg['title'], $permalink_cfg['script'], $id, $permalink_cfg['param'], $permalink_cfg['folder'], $permalink_cfg['auto'], true);

	// cache
	if (!empty ($cfg['auto_recache']) || !empty ($cfg['recache']) || !empty ($cfg['rebuild_cache'])) qcache_clear ();
...
function qadmin_search ($def, $cfg)
{
	...
	foreach ($foo as $hkey => $hval) $cfg['hidden_value'] .= "<input type=\"hidden\" name=\"$hkey\" value=\"$hval\" />\n";
...
function qadmin_search_result ($def, $cfg, $list_mode = FALSE)
{
	...
	if (!empty ($jj)) $cfg['search_key'] = $cfg['search_key'].','.implode (',', $jj);
	...
	foreach ($k as $val)
	{
		...
		if (!empty ($def[$fiel]['format'])) $format = $def[$fiel]['format']; else $format = 'default';
		if (substr ($format, 0, 7) == 'numeric') $t['align'] = 'right';
		elseif ($format == 'currency') $t['align'] = 'right';
	...
	// date param
	if ($andor == 'and') $andor = 'AND'; else $andor = 'OR';
	if (!empty ($start) && empty ($end)) $sql_where .= " $andor ($cfg[search_date_field] >= '$start')";
	if (!empty ($start) && !empty ($end)) $sql_where .= " $andor ($cfg[search_date_field] >= '$start' AND $cfg[search_date_field] <= '$end')";
	...
	// search!
	if ($orderby == 'd') $orderby = 'DESC'; else $orderby = 'ASC';
	if (empty ($sortby)) $sss = ''; else $sss = "$sortby $orderby";				// sql sort method
	...
	foreach ($result as $val)
	{
		...
		foreach ($val as $key => $value)
		{
			...
			{
				// get field type
				foreach ($def as $t) if ($t['field'] == $key)
				{
					$type = $t['type'];
					if (!empty ($t['format'])) $format = $t['format']; else $format = 'default';
				}
				...
				if (empty ($type)) $type = '';
				if (($type == 'wysiwyg') || ($type == 'text') || ($type == 'code')) $t['result'] = line_wrap (strip_tags ($t['result']), 200);
				...
				if ($format == 'date') { $t['result'] = convert_date ($t['result']); }
				if ($format == 'currency') { $t['result'] = num_format ($t['result'], 0, 1); $t['align'] = 'right'; }
				...
				// apply url mask
				if (!empty ($result_url[$key])) $t['result'] = sprintf ($lang['l_open_url'], str_replace ('__KEY__', $t['result'], $result_url[$key]), $t['result']);

				// output
				if (substr ($t['result'], 0, 6) == 'guest*') $t['result'] = '(Guest)';

				// see if current result row has sub_result
				$j_idx = array_search ($key, $j);
				if (!empty ($jj[$j_idx])) $t['result'] = '<b>'.$t['result'].'</b><br />'.$val[$jj[$j_idx]];
		...
		if ($cfg['cmd_update_enable'])
		{
			...
			else
			{
				...
				if (!empty ($cfg['search_edit_target'])) $t['edit_target'] = $cfg['search_edit_target'];
			}
	...
	if (!empty ($cfg['cmd_new_enable'])) $cfg['new_item_form'] = quick_tpl ($tpl_section['qadmin_new_item'], $cfg);

	$result = quick_tpl ($tpl_section['qadmin_search_result'], $cfg);

	// additional header & footer
	if (!empty ($cfg['footer'])) $result = $result.$cfg['footer'];
...
{
	global $config;
	if ($config['demo_mode']) admin_die ('admin_ok');
	...
	// get field type
	foreach ($def as $t) if ($t['field'] == $field) { $type = $t['type']; $prefix = (!empty ($t['prefix'])) ? $t['prefix'] : ''; }
	...
	if (!empty ($row[$field]) || ($type == 'img_series') || ($type == 'img_set'))
	{
		...
		// remove file
		$fn = $row[$field];
		if ($type == 'file') @unlink ($cfg['file_folder'].'/'.$fn);
		if (($type == 'img') || ($type == 'image') || ($type == 'img_resize') || ($type == 'image_resize')) @unlink ($cfg['img_folder'].'/'.$fn);
		if ($type == 'thumb') { @unlink ($cfg['img_folder'].'/'.$fn); @unlink ($cfg['thumb_folder'].'/'.$fn); }
		...
		if (!empty ($cfg['enable_log']))
		{
			...
			if ($old != $new)
			{
				if (empty ($cfg['detailed_log'])) { $foo = $old[$cfg['log_title']]; $old = $new = ''; $old[$cfg['log_title']] = $foo; }
...
function qadmin_remove_item ($def, $cfg)
{
	...
	if (empty ($pv_arr)) $pv_arr = array ($primary_val);
	...
	foreach ($pv_arr as $primary_val)
	{
		...
		{
			@$fn = $row[$val['field']];
			if ($val['type'] == 'file') @unlink ($cfg['file_folder'].'/'.$fn);
			if (($val['type'] == 'img') || ($val['type'] == 'image') || ($val['type'] == 'img_resize') || ($val['type'] == 'image_resize')) @unlink ($cfg['img_folder'].'/'.$fn);
			if ($val['type'] == 'thumb') { @unlink ($cfg['img_folder'].'/'.$fn); @unlink ($cfg['thumb_folder'].'/'.$fn); }
		...
		if (!empty ($cfg['enable_log'])) $log_info = sql_qquery ("SELECT $cfg[log_title] FROM $cfg[table] WHERE $cfg[primary_key] = '$primary_val' LIMIT 1");
...
function qadmin_manage ($def, $cfg, $title = array (), $tpl_mode = 'full')
{
	...
	// security
	if ($cfg['admin_level']) admin_check ($cfg['admin_level']);

	$cmd = get_param ('qadmin_cmd');
	$id = get_param ('id');

	//  get cmd from POST
	if (empty ($cmd)) $cmd = post_param ('qadmin_cmd');

	// manage cmd
	if (empty ($cmd)) $cmd = $cfg['cmd_default'];
	if (!empty ($id)) $cmd = 'update';
	if (post_param ('qadmin_process')) $cmd = 'process';

	// logging
	if (!isset ($cfg['enable_log'])) $cfg['enable_log'] = $config['enable_qadmin_log'];
	if (!isset ($cfg['detailed_log'])) $cfg['detailed_log'] = $config['enable_detailed_log'];
	if (empty ($cfg['log_title'])) $cfg['enable_log'] = $cfg['detailed_log'] = '';

	// tpl mode
	if ($tpl_mode == 'popup') $tpl_mode = 'adm_popup'; else $tpl_mode = 'adm';
