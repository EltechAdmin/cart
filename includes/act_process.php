...
if (empty ($row['user_id'])) $err[] = 'User ID not found';
if (empty ($row['user_activation'])) $err[] = 'This account has been activated';
if (empty ($act) || $act != $row['user_activation']) $err[] = 'Invalid activation key';

// if error -> HALT!
if (!empty($err)) msg_die (sprintf ($lang['msg']['act_error'], '<ul><li>'.implode ('</li><li>', $err).'</li></ul>'));
