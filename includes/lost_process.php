...
if ($do_reset)
{
	// check entry
	$err = array ();
	if ($visual != qvc_value ()) msg_die ($lang['msg']['captcha_error']);
	if (empty ($user_id) || empty ($reset) || empty ($user_passwd)) $err[] = 'Invalid entries';

	// get user id
	$row = sql_qquery ("SELECT * FROM ".$db_prefix."user WHERE user_id='$user_id' LIMIT 1");
	if (empty ($row['user_id']) || empty ($row['reset_passwd']) || ($row['reset_passwd'] != $reset)) msg_die (sprintf ($lang['msg']['reset_error'], '<ul><li>'.implode ('</li><li>', $err).'</li></ul>'));
...
else
{
	...
	if (empty($row['user_id'])) msg_die ($lang['msg']['lost_error']);
