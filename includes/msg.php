...
$r = get_param ('r');
if (empty ($full)) $full = false;
if (empty ($popup)) $popup = false;
if (empty ($admin)) $admin = false;
if (empty ($mini)) $mini = false;	// if true indicates that popup message should use mini popup, which is: very simple (no whistles, etc), and disappear in 1 second, aka unobtrusive message box) -- see OK case
// $msg_id = strtolower ($msg_id);
if (strpos (cur_url (), '%2F'.$config['admin_folder'].'%2F')) $admin = true;;
...
if (empty ($url))
{
	...
	else
	{
		...
		if (!strpos ('.'.$url, $config['site_url'])) $url = $config['site_url'];	// do NOT redirect to external page
	}
}
...
if (empty ($msg_id) && !empty ($msg_txt)) $msg_id = 'echo';
