...
function admin_die ($msg_txt, $url = '')
{
	...
	$admin = true;
	if ($msg_txt == 'admin_ok') { $mini = true; $msg_txt = $lang['msg']['admin_ok']; }
	elseif ($msg_txt == 'demo_mode') $msg_txt = $lang['msg']['demo_mode'];
...
function generate_permalink ($title, $scriptname, $targetidx, $targetparam = '', $folder = '', $auto = false, $write = false)
{
	...
	if ($auto)
	{
		...
		while (!$ok)
		{
			$i++;
			$jj = "$folder/$j.php";
			if ($jj[0] == '/') $jj = substr ($jj, 1);
			$pfoo = sql_qquery ("SELECT * FROM ".$db_prefix."permalink WHERE (url='$jj') LIMIT 1");
			if (empty ($pfoo['idx'])) $ok = true; else $j = $t.'-'.$i;
		}

		if ($config['adp_extension']) $t = $j.'.'.$config['adp_extension']; else $t = $j;
	...
	{
		// update
		$new = false; $old_id = $foo['idx']; $old_url = $foo['url'];

		// is permalink changed?
		if ($foo['url'] != $permalink) $permalink_changed = true; else $permalink_changed = false;
		$exists = sql_qquery ("SELECT * FROM ".$db_prefix."permalink WHERE (url='$permalink') AND (idx!='$foo[idx]') LIMIT 1");
	}

	if (!empty ($exists)) return false; else $result = $permalink;	// not unique!
...
function add_new_language ($lang_id, $lang_key, $lang_value)
{
	...
	while ($row = sql_fetch_array ($res))
	{
		...
		{
			if ($lang_id == $l_id) sql_query ("UPDATE ".$db_prefix."language SET lang_value='$l_val' WHERE lang_id='$l_id' AND lang_key='$l_key' LIMIT 1");
		}
	}
}
...
function qadmin_log ($pid, $title, $action, $old, $new, $table)
{
    ...
    if (!empty ($old)) $old = base64_encode (gzcompress (serialize ($old)));
    if (!empty ($new)) $new = base64_encode (gzcompress (serialize ($new)));
    if (is_array ($new)) $new = '';
    $ip = get_ip_address ();
