...
// get param
if (!$isLogin) msg_die ($lang['msg']['not_member']);
...
if ($current['user_email'] != $user_email)
{
	...
	if (qhash ($user_passwd) != $current['user_passwd']) $err[] = $lang['l_password_error'];
	if (!validate_email_address ($user_email)) $err[] = $lang['l_email_empty'];

	// email exists?
	$row = sql_qquery ("SELECT user_email FROM ".$db_prefix."user WHERE user_email = '$user_email' LIMIT 1");
	if (!empty($row['email'])) $err[] = $lang['l_email_used'];

	// error
	if (!empty ($err)) msg_die (sprintf ($lang['msg']['update_error'], '<ul><li>'.implode ('</li><li>', $err).'</li></ul>'));
