...
function get_skin_path ($mode, $skinfile, $required = true)
{
	...
	switch ($mode)
	{
		...
		case 'mod_admin':
		 ...
		 if (!file_exists ($path)) $notFound = TRUE;
		break;
		...
		case 'mod':
		 ...
		 if (!file_exists ($path))
		 {
			...
			if (!file_exists ($def_path)) $notFound = TRUE;
			$path = $def_path;
		 }
		break;
...
function load_tpl ($mode, $skinfile = '', $required = true, $raw = false)
{
	...
	// get skin path
	if (empty ($skinfile)) $skinfile = $mode;
	if ($mode == 'var')
		$tpl = $skinfile;
	elseif ($mode == 'mail')
	{
	...
	else
	{
		...
		if ($path === false) { $debug_info['tpl'][] = $skinfile.' -&gt; <font color="#ff0000">Load failed!</font>'; return false; }
...
function quick_tpl ($tpl, $text)
{
	...
	foreach ($matches[1] as $val) $tpl = str_replace ('{$'.$val.'}', $lang[$val], $tpl);
...
function init_module ($args)
{
	...
	if (!$config['enable_module_engine']) return ('<!-- MODULE_ENGINE IS DISABLED -->');
	...
	if (empty ($x)) { $debug_info['mod'][] = $mod_id.' -&gt; <style="color:#f00">Not Enabled</style>'; return "<!-- module $mod_id is disabled or not installed! -->"; }

	// find module
	if (!@file_exists ('./module/'.$mod_id.'/window.php'))
	{ $debug_info['mod'][] = $mod_id.' -&gt; <style="color:#f00">Not Found</style>';  return "<!-- module $mod_id is not available -->"; }
	...
	{
		if (!empty ($mod_ini['mod_pos'])) $pos_url = $config['site_url'].'/'.$config['admin_folder'].'/manage.php?highlight='.$mod_ini['mod_pos']; else $pos_url = "javascript:alert('This module was added manually without using module manager!')";
		if (!empty ($mod_content_edit_url)) $content_url = $mod_content_edit_url; else $content_url = "javascript:alert('This module doesn\'t support such feature!')";
...
function flush_tpl ($mode = 'user', $subtpl = '')
{
	...
	if ($print) $mode = 'print';
	if (empty ($adm_popup)) $adm_popup = post_param ('adm_popup');
	if ($adm_popup) $mode = 'adm_popup';
	...
	{
		// load sub tpl
		if (empty ($subtpl) && ($mode != 'user')) $subtpl = $mode;
		if (empty ($subtpl)) $subtpl = 'body_default.tpl';
		...
		if (!$subtpl) $subtpl = load_tpl ('body_default.tpl');
	...
	if ($mode == 'user')
	{
		...
		if ($config['enable_module_man'])
		{
			...
			foreach ($short as $val) $long[] = 'module_box_'.$val;
			$qc = qcache_get ($long);
			...
			foreach ($qc as $k => $v)
			{
				if ($qc[$k] === false) $cache_avail = false;
				$txt[$k] = $v;
			}
			if (!$cache_avail)
			{
				...
				foreach ($short as $k => $v) qcache_update ('module_box_'.$v, $txt['module_box_'.$v]);
			}
		}
		else
		{
			...
			$short = array ('T1', 'T2', 'L1', 'L2', 'R1', 'R2', 'B1', 'B2');	// list of positions
			foreach ($short as $val) $txt['module_box_'.$val] = '<!-- MODULE_MAN is disabled -->';
		}
	}
	...
	if (($mode != 'admin') && ($mode != 'adm') && ($mode != 'adm_popup')) $content = preg_replace_callback ("/{qemod:(.*?)}/is", 'init_module', $content);
	...
	$enc = '';
	reset_form ();
	if (empty ($_SERVER["HTTP_ACCEPT_ENCODING"])) $_SERVER["HTTP_ACCEPT_ENCODING"] = '';
	...
	{
		if (!headers_sent ()) header ("Content-Type: text/html; charset=".$lang['l_encoding']);
		echo $content;
	}
	...
	if ($config['debug_mode'])
	{
		...
		$time_end = getmicrotime();
		if (function_exists ('memory_get_usage')) $mfinish = memory_get_usage (); else $mfinish = 0;
		if (function_exists ('memory_get_peak_usage')) $mpeak = memory_get_peak_usage(); else $mpeak = 0;
		...
		foreach ($debug_info['sql'] as $k => $v) echo "<tr><td width=\"5%\" valign=\"top\">$k.</td><td width=\"95%\">$v</td></tr>\n";

		echo "<tr><td colspan=\"2\"><b>Module Information</b></td></tr>\n";
		foreach ($debug_info['mod'] as $k => $v) echo "<tr><td width=\"5%\">$k.</td><td width=\"95%\">$v</td></tr>\n";

		echo "<tr><td colspan=\"2\"><b>Template Information</b></td></tr>\n";
		foreach ($debug_info['tpl'] as $k => $v) echo "<tr><td width=\"5%\">$k.</td><td width=\"95%\">$v</td></tr>\n";
