...
// digital product?
if ($item['digital_file']) $digital_product = true;

// stock
if ($item['stock']) $is_stock = true;

// discount
if (($item['price'] < $item['price_msrp']) && !$item['is_call_for_price']) $discount_status = true;

// subproducts
if ($item['sub_product']) $sub_product = $item['is_sub_product'] = true;
...
if (!empty ($max_buy) || ($min_buy != 1)) $qty_more = true;

// multier price
$pq = unserialize ($item['price_qty']);
if (!empty ($pq)) $multier = true;
...
if (!$content)
{
	...
	// if no image
	if ($i == 1) $item['block_thumb'] = "<img border=\"0\" src=\"$config[skin]/images/nothumb_detail.png\" alt=\"image\" />";

	// bread crumb
	$item['block_cat_bread_crumb'] = '';
	foreach ($item['bread_cat'] as $k => $v) $item['block_cat_bread_crumb'] .= quick_tpl ($tpl_block['cat_bread_crumb'], $v);

	// multi tier
	$item['multier_list'] = '';
	if ($multier) foreach ($pq as $k => $v)
	{
	...
	// wish?
	$current_f = array ();
	if (!empty ($current_user_info['user_wish'])) $current_f = explode (',', $current_user_info['user_wish']);
	if (!in_array ($item_id, $current_f)) $add_wish = true;
...
if ($current_admin_level) $txt['main_body'] = sprintf ($lang['edit_product_in_acp'], $item_id).$txt['main_body'];
flush_tpl ();
?>