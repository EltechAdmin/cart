...
switch ($cmd)
{
	case 'userOk':
		if (!empty ($q))
		{
			...
			if (!empty ($foo) || !preg_match ("/^[[:alnum:]]+$/", $q)) flush_json (0) ; else flush_json (1);	// 1 = username is ok
		}
	break;
	...
	case 'emailOk':
		if (!empty ($q))
		{
			...
			if (!empty ($foo) || !validate_email_address($q)) flush_json (0); else flush_json (1);	// 1 = email is ok
		}
	break;
