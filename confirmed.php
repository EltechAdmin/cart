...
if (!$pay) msg_die ($lang['msg']['payment_not_selected']);
...
if (!$ship) msg_die ($lang['msg']['shipper_not_selected']);
...
// update coupon/gift
if ($cart['gift_code']) sql_query ("UPDATE ".$db_prefix."gift SET redeem_order_id='$order_id', redeem_date=UNIX_TIMESTAMP(), redeem_purchase='$cart[total]' WHERE gift_code='$cart[self_gift_code]' LIMIT 1");

// prepare text for email to buyer & seller
$txt = array_merge ($txt, $user);
if (empty ($ship['fee'])) $txt['shipping_cost'] = $lang['l_courier_free']; else $txt['shipping_cost'] = num_format ($ship['fee'], 0, 1);
if (empty ($pay['fee'])) $txt['payment_cost'] = $lang['l_courier_free']; else $txt['payment_cost'] = num_format ($pay['fee'], 0, 1);
