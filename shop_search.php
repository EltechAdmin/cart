...
function filter_by_cf ($query = '', $cat_id)
{
	...
	foreach ($ce_cache['cf_define'] as $row)
	{
		...
		switch ($row['cf_type'])
		{
			case 'varchar':
			case 'textarea':
				if (!empty ($val) && $row['is_searchable']) $sql[$k] = "(t2.$key='$val')";
			break;
			...
			case 'multi':
				...
				if (!empty ($selected))
				{
					...
					{
						$daa = array ();
						foreach ($selected as $k => $v) $daa[] = "(t2.$key LIKE '%\r\n$v\r\n%')";
...
// get parameters
if ($isPermalink) { $_GET['cat_id'] = $original_idx; $_GET['cmd'] = 'list'; }
...
if (!empty ($query)) $cmd = 'search';
if (empty ($query) && empty ($cat_id)) $cmd = 'search';
...
if ($cmd == 'list')
{
	...
	if (empty ($cat_inf)) msg_die ($lang['msg']['cat_error']);
	if ($cat_inf['cat_page'])
	{
...
if ($cat_id)
{
	if (empty ($cat_inf)) $cat_inf = sql_qquery ("SELECT * FROM ".$db_prefix."product_cat WHERE idx='$cat_id' LIMIT 1");
...
if ($query)
{
	$list_only = false;
	$w = array ();
	if (!$cfonly) $w[] = create_where ('smart_search', $query);
	if (!empty ($cf_sql['sql_query'])) $w[] = $cf_sql['sql_query'];
	$sql[] = '('.implode (' OR ', $w).')';
}
...
if (empty($s) or !array_key_exists($s, $sortby)) $s = $ds;
if (empty($o) or !array_key_exists($o, $sortorder)) $o = $do;
...
$i = 0;
unset ($_GET['ajax']);
if ($isPermalink) unset ($_GET['mod_id'], $_GET['cmd'], $_GET['cat_id']);
...
if (!$i) $no_search_result = true; else $no_search_result = false;
...
if ($cmd == 'list')
{
	...
	$txt['block_cat_list'] = '';
	if (!empty ($cat_inf['cat_image'])) $txt['cat_image'] = "<img src=\"$config[site_url]/public/image/$cat_inf[cat_image]\" alt=\"image\" />"; else $txt['cat_image'] = $cat_inf['cat_name'];

	// cat list
	$txt['block_cat_list'] = '';
	$foo = create_cat_list ($cat_id);
	foreach ($foo as $val) $txt['block_cat_list'] .= quick_tpl ($tpl_block['cat_list'], $val);
	...
	foreach ($foo as $v)
	{
		...
		if ($v != $cat_id) $txt['block_cat_bread_crumb'] .= quick_tpl ($tpl_block['cat_bread_crumb'], $bc);
	}

	// show feat listing on first page && when list only (no filtering, search, etc)
	if ((!empty ($cat_inf['cat_featured'])) && ($list_only) && ($p == 1)) $featured_listing = true;
...
// add acp shortcut
if (($current_admin_level) && empty ($query) && !empty ($cat_inf)) $txt['main_body'] = sprintf ($lang['edit_product_cat_in_acp'], $cat_id).$txt['main_body'];
