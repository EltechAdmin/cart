...
if (!$isLogin) redir ($config['site_url']);
...
switch ($cmd)
{
	case 'download':
		...
		if (empty ($row)) msg_die ($lang['msg']['no_file']);

		// check file
		$prdo = sql_qquery ("SELECT * FROM ".$db_prefix."products WHERE idx='$item_id' LIMIT 1");
		if (empty ($prdo['digital_file'])) msg_die ($lang['msg']['no_file']);
		...
		if (!file_exists ($source)) msg_die (sprintf ($lang['msg']['internal_error'], 'File not available.'));
