...
if ($view) $mode = 'view'; else $mode = 'enter';
...
switch ($mode)
{
	case 'enter':
		...
		// single use
		if (($row['coupon_type'] == 'once') && (($row['redeem_user_id'] != '') || ($row['redeem_order_id'] != ''))) msg_die ($lang['msg']['coupon_err']);

		// update coupon -> change ownership to $current_user_id
		if ($sql_today > $row['valid_date']) msg_die ($lang['msg']['coupon_err']);
		...
		if ($row['coupon_type'] == 'once') sql_query ("UPDATE ".$db_prefix."gift SET redeem_user_id='$current_user_id' WHERE gift_code='$code' LIMIT 1");
	...
	case 'view':
		...
		if (empty ($row)) msg_die ($lang['msg']['coupon_err']);
