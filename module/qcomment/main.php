...
function format_comment (&$row, $rating_box, $conc)
{
	...
	// rating?
	if (($rating_box) && ($row['comment_rate'])) $row['rating'] = rating_img ($row['comment_rate'], 12); else $row['rating'] = '';
...
function get_conc ($i)
{
	...
	{
		if (empty ($crow['comment_user'])) $crow['comment_user'] = $lang['l_guest'];
...
if ($hold && $current_admin_level)
{
	...
	if (empty ($key) && $success) flush_json (1);		// if success (and removed from UI) => return 1
	elseif (empty ($key) && !$success) flush_json (9999, 'Could not hold comment right now. Please try again!');	// if failed (and removed from UI) => return json false
	else redir ();
}
if ($approve && ($current_admin_level || (qhash ('qcomment_ok_'.$approve) == $key)))
{
	...
	if (empty ($key) && $success) flush_json (1);		// if success (and removed from UI) => return 1
	elseif (empty ($key) && !$success) flush_json (9999, 'Could not approve comment right now. Please try again!');	// if failed (and removed from UI) => return json false
	else redir ();
}
if ($trash && ($current_admin_level || (qhash ('qcomment_no_'.$trash) == $key)))
{
	...
	if (empty ($key) && $success) flush_json (1);		// if success (and removed from UI) => return 1
	elseif (empty ($key) && !$success) flush_json (9999, 'Could not remove comment right now. Please try again!');	// if failed (and removed from UI) => return json false
	else redir ();
}
...
if (!$isLogin) $captcha = true;												// guest must enter captcha
...
switch ($mode)
{
	...
	case 'show_comment':
	case 'show_box':
	 if (empty ($title)) msg_die ($lang['msg']['qcomment_err_2']);

	 // open tpl
	 if ($member_only && !$isLogin) $show_box = FALSE; else $show_box = TRUE;
	 if ($unique)
	 {
	 ...
	 if ($mode == 'show_comment')
	 {
		...
		if (!$current_admin_level) $txt['block_comment'] = qcache_get ('qcomment_main_'.$m.'_'.$i); else $txt['block_comment'] = '';	// disable cache for admin, thus it will display admin tools
		if (empty ($txt['block_comment']))
		{
			...
			if (!$current_admin_level) qcache_update ('qcomment_main_'.$m.'_'.$i, $num.'|'.$txt['block_comment']);
		}
		else
		{
	...
	case 'save_comment':
	 // member only?
	 if ($member_only && !$isLogin) msg_die ($lang['msg']['not_member']);

	 // unique? (NEED MEMBER ONLY RULE)
	 if ($unique)
	 {
		$foo = sql_qquery ("SELECT comment_user FROM ".$db_prefix."qcomment WHERE mod_id='$m' AND item_id='$i' AND comment_user='$current_user_id' LIMIT 1");
		if (!empty ($foo)) msg_die ($lang['msg']['qcomment_err_3']);
	 }
	 ...
	 // verify entries
	 if (($captcha) && (qhash ($visual) != qvc_value ())) msg_die ($lang['msg']['captcha_error']);
	 if (!$rating_box) $comment_rate = 0;
	 if (($comment_rate < 0) || ($comment_rate > 5) || (!is_numeric ($comment_rate))) $comment_rate = 0;
	 if (empty ($comment_body) || (strlen ($comment_body) < 1)) msg_die ($lang['msg']['qcomment_err_4']);
	 if (empty ($comment_title)) $comment_title = $lang['l_untitled'];
	 if (!$isLogin) $comment_user = $lang['l_guest']; else $comment_user = $current_user_id;
	...
	case 'save_rate':
	 ...
	 if (!empty ($cookies[$ident])) msg_die ($lang['msg']['qcomment_err_5']);
	 if (($rate > 5) || ($rate < 1)) msg_die ($lang['msg']['qcomment_err_6']);
	...
	case 'helpful':
	 ...
	 if (!empty ($cookies[$c])) msg_die ($lang['msg']['qcomment_err_5']);
