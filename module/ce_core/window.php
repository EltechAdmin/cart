...
if (empty ($mod_ini['mode'])) $mod_ini['mode'] = '';
...
switch ($mod_ini['mode'])
{
	case 'product_list':
		...
		if (!empty ($mod_ini['display_overwrite'])) $display = $mod_ini['display_overwrite'];
		if (!empty ($mod_ini['csswrapper_grid']) && $display == 'grid') $csswrapper = $mod_ini['csswrapper_grid'];
		if (!empty ($mod_ini['csswrapper_list']) && $display == 'list') $csswrapper = $mod_ini['csswrapper_list'];
		...
		{
			if ($isPermalink) { $_GET['cat_id'] = $original_idx; }
		...
		elseif ($items == 'history')
		{
			...
			$i = 0; $foo = array (); $count = count ($history);
			foreach ($history as $k => $v) if (is_numeric ($v)) $foo[] = $v;
			...
			// update cookies
			$item_id = get_param ('item_id');
			if ($item_id && !in_array ($item_id, $foo) && is_numeric ($item_id)) setcookie ($db_prefix."history[$count]", $item_id, 0, '/', cookie_domain ());
		}
		elseif ($items == 'wish')
		{
			if ($isLogin)
		...
		else $sql_where = "t1.idx IN ($items)";

		// load tpl
		$tpl = load_tpl ('mod', 'module_ce_core_list.tpl');
		if ($display == 'grid') $tpl = $tpl_section['list_gridbox'];
		elseif ($display == 'list') $tpl = $tpl_section['list_listbox'];
		else $tpl = $tpl_section['list_list'];
		...
		if (!$out_of_stock) $where[] = '(stock > 0)';
		if ($sql_where) $where[] = "($sql_where)";
