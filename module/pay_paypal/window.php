...
switch ($payment_cmd)
{
	case 'form':
		...
		$form['method'] = 'post';
		if ($module_config['pay_paypal']['sandbox']) $form['action'] = 'https://www.sandbox.paypal.com/cgi-bin/webscr';

		// you need also to return how to pay information (if any)
		$form['txt_howtopay'] = '';

		// conversion rate
		if (empty ($module_config['pay_paypal']['conversion_rate'])) $cv = 1; else $cv = 1 / $module_config['pay_paypal']['conversion_rate'];
		...
		if ($module_config['pay_paypal']['ipn']) $hidden_val['notify_url'] = $config['site_url'].'/module/pay_paypal/pp_ipn.php';

		$form['hidden'] = '';
		foreach ($hidden_val as $key => $val) $form['hidden'] .= "<input type=\"hidden\" name=\"$key\" value=\"$val\" />\n";
	break;
