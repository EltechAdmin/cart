...
$mypayment_status = 'E';
$url = 'www.paypal.com';
if ($module_config['pay_paypal']['sandbox']) $url = 'www.sandbox.paypal.com';
...
else
{
	...
	while (!feof($fp))
	{
		...
		if (stripos($res, "VERIFIED") !== false)
		{
			...
			// get conclusion
			if ($ps1 && $ps2 && $ps3 && $ps4)
				$mypayment_status = 'P';	// SUCCESS (PAID)
			else
				$mypayment_status = 'E';	// PENDING
		}
...
if (!$order_info) $order_id = 'Invalid Order ID';
if ($mc_gross < $total) { $mypayment_status = 'E'; $notes[] = 'Total payment less than invoice!'; }
$notes_log = implode ("\r\n", $notes);
