...
if (empty ($toOutput)) $toOutput = $mod_raw;
$output = str_replace ('__SITE__', $config['site_url'], html_unentities (stripslashes ($toOutput)));

// content editor
if (!empty ($mod_ini['mod_pos'])) $mod_content_edit_url = $config['site_url'].'/'.$config['admin_folder'].'/manage.php?highlight='.$mod_ini['mod_pos'];
?>