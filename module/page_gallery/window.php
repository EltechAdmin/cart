<?php
if (empty ($mod_ini['page_id']) && $inline) { $mod_ini['page_id'] = $mod_raw; $mod_ini['body'] = 1; }
...
if ($page_id)
{
	if (!$limit) $limit = substr_count ($page_id, ',') + 1;
...
if (empty ($page_id)) $sql .= " AND page_list != 0 AND page_status = 'P'";
...
{
	$tpl = '<div class="{$class}">';
	if (($imgpos == 'top') && $img) $tpl .= '<div>{$page_image}</div>';
	if ($title) $tpl .= '<h2>{$page_title}</h2>';
	if (($imgpos == 'bottom') && $img) $tpl .= '<div>{$page_image}</div>';
	if ($thumb) $tpl .= '<div style="float:left; margin-right:5px; margin-bottom:5px">{$page_thumb}</div>';
	if ($body) $tpl .= '<div>{$page_body}</div>';
	if ($summary) $tpl .= '<div>{$page_summary}</div>';
	if ($date) $tpl .= '<p>Posted on: {$page_date}</p>';
	if ($more) $tpl .= '<span class="glyphicon glyphicon-chevron-right"></span> <a href="{$url}">More</a>';
	if ($thumb) $tpl .= '<div style="clear:both"></div>';
	if (($limit == 1) && (!$hr)) $tpl .= '<hr />';
	$tpl .= '</div>';
}
...
while ($row = sql_fetch_array ($res))
{
	switch ($style)
	{
		...
		case 'list':
			...
			if ($config['enable_adp'] && $row['permalink']) $url = $config['site_url'].'/'.$row['permalink']; else $url = $config['site_url']."/page.php?pid=$row[page_id]";
			$output .= "<li>";
			if ($thumb) $output .= $row['page_thumb'].' ';
			if ($title) $output .= "<a href=\"$url\">$row[page_title]</a>";
			$output .= "</li>\n";
		break;
