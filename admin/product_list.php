...
switch ($cmd)
{
	case 'qEdit':
		$items = '';
		foreach ($_GET as $k => $v)	if (substr ($k, 0, 7) == 'select_')
		{
			$kk = substr ($k, 7);
			$items .= $kk.',';
		}
	...
	default:
		if ($cmd == 'search')
		{
			...
			if (!empty ($cat_id)) $sql_arr[] = "(cat_id='$cat_id') OR (add_category LIKE '%,$cat_id,%')";

			if (!empty ($start)) $sql_arr[] = "(list_date >= '$start') AND (list_date <= '$end')";

			if (!empty ($items)) $sql_arr[] = "idx IN ($items)";
