...
if (!$config['fman_imagelib_enable']) just_die ($lang['msg']['fman_not_allowed']);
...
if (!chdir ($cur_dir)) just_die ($lang['msg']['fman_not_allowed']);
...
switch ($cmd)
{
	...
	case 'del':
		...
		if (file_exists ($config['abs_path'].'/public/thumb/'.$fn)) unlink ($config['abs_path'].'/public/thumb/'.$fn);
		redir ();
	break;
	...
	case 'upload':
		...
		if ($compress)
		{
			if (!empty ($config['watermark_file'])) image_watermark ($tmp, $config['abs_path'].'/public/image/'.$config['watermark_file']);
			image_optimizer ($tmp, $cur_dir.'/'.$fn, $config['optimizer']);
		}
		else
		{
			$foo = upload_file ('upload', $cur_dir.'/'.$fn);
			if ($foo['success'] && $foo['count']) $tgt = $foo[0]['filename'];
			@chmod ($cur_dir.'/'.$tgt, 0644);
			if (!empty ($config['watermark_file'])) image_watermark ($cur_dir.'/'.$tgt, $config['abs_path'].'/public/image/'.$config['watermark_file']);
		}
		redir ();
	break;
	...
	default:
		// load tpl
		$view = get_param ('view');
		if ($view == 'list') $tpl_mode = 'list'; else $tpl_mode = 'thumb';
		$tpl = load_tpl ('etc', $config['fman_skin'].'/imagelib.tpl'); // <-- modify if necess.
		...
		if (!$handle) just_die ($lang['msg']['fman_not_allowed']);
		...
		// output
		while (list ($key, $val) = each ($file_array)) $txt['block_fileman_item'] .= quick_tpl ($tpl_block['fileman_item'], $file_array[$key]);
		$txt['cur_dir'] = $cur_dir;
