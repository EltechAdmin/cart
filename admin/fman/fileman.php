...
function get_type ($fn)
{
	...
	if (in_array ($fx, $t))	return 'txt';
	if (in_array ($fx, $i))	return 'img';
	return 'file';
}
...
// security
if ($cur_path == $top_path) $in_top_path = 1; else $in_top_path = 0;
...
switch ($cmd)
{
	case 'copy':
	 ...
	 if (file_exists ($fname_new)) just_die ($lang['msg']['fman_copy_err']);

	 // copy
	 if (!@copy ($fname_old, $fname_new)) just_die ($lang['msg']['fman_copy_err']);
	...
	case 'move':
	 ...
	 // is file exists?
	 if (file_exists ($fname_new)) just_die ($lang['msg']['fman_move_err']);

	 // move
	 if (!@copy ($fname_old, $fname_new)) just_die ($lang['msg']['fman_move_err']);
	 if (!@unlink ($fname_old)) just_die ($lang['msg']['fman_move_err']);
	 @chmod ($fname_new, 0644);
	...
	case 'ren':
	 ...
	 // rename
	 if (!@rename ($fname_old, $fname_new)) just_die ($lang['msg']['fman_ren_err']);
	 redir ("$config[fman_path]/fileman.php?chdir=$chdir");
	break;
	...
	case 'del':
	 $fname = $cur_path.'/'.$fn;
	 if (@!unlink ($fname))	just_die ($lang['msg']['fman_del_err']);
	 redir ("$config[fman_path]/fileman.php?chdir=$chdir");
	break;
	...
	case 'new':
	 ...
	 // check if file exists
	 if (file_exists ($fname)) just_die ($lang['msg']['fman_new_err']);

	 // create new file
	 if (!$fp = @fopen ($fname, 'w')) just_die ($lang['msg']['fman_new_err']);
	 fclose ($fp);
	 @chmod ($fname_new, 0644);
	 redir ("$config[fman_path]/fileman.php?chdir=$chdir");
	break;
	...
	case 'mkdir':
	 $fname = $cur_path.'/'.$fn;
	 if (!@mkdir ($fname)) just_die ($lang['msg']['fman_mkdir_err']);
	 redir ("$config[fman_path]/fileman.php?chdir=$chdir");
	break;
	...
	case 'rendir':
	 ...
	 $fname_old = $cur_path.'/'.$fn_old;
	 $fname_new = $cur_path.'/'.$fn_new;
	 if (!@rename ($fname_old, $fname_new)) just_die ($lang['msg']['fman_rendir_err']);
	 redir ("$config[fman_path]/fileman.php?chdir=$chdir");
	break;
	...
	case 'rmdir':
	 $fname = $cur_path.'/'.$fn;
	 if (!@rmdir ($fname)) just_die ($lang['msg']['fman_rmdir_err']);
	 redir ("$config[fman_path]/fileman.php?chdir=$chdir");
	break;
	...
	default:
	 ...
	 if (!$handle) just_die ($lang['msg']['fman_not_allowed_err']);
	 while (false !== ($fn = readdir($handle)))
	 {
		$s = stat ($cur_path.'/'.$fn);
		$f_size = num_format ($s['size']);
		$f_mtime = date ("m-d-Y g:i a", $s['mtime']);
		if (is_dir ($cur_path.'/'.$fn)) $dir = 1; else $dir = 0;
		...
		else
		{
			...
			if (get_type ($fn) == 'txt') $file_array[$f]['tools'] .= "<a href=\"edit.php?chdir=$chdir&amp;fn=$fn\" class=\"popiframe\">$fman_lang[fman_edit]</a>";
		}
	 ...
	 // output
	 while (list ($key, $val) = each ($dsp)) $txt['block_fileman_item'] .= quick_tpl ($tpl_block['fileman_item'], $dsp[$key]);
