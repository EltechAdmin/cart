...
if ($config['demo_mode']) admin_die ($lang['msg']['demo_mode']);
// set vars
...
if (empty ($fn)) $fn = basename (post_param ('fn'));
if ($chdir == '/') $chdir = '';
...
if (!file_exists($fl)) admin_die ($lang['msg']['qadmin_item_not_found']);
...
else
{
	...
	$fp = fopen ($fl,'r');
	while(!feof($fp)) $string .= fgets($fp,4096);
	fclose ($fp);
