...
switch ($cmd)
{
	case 'do_backup':
	 // demo mode?
	 if ($config['demo_mode']) die ('Demo Mode! Back up is cancelled!');
	 html_header ();
	 ...
	 {
		$t = $row[0];
	 	if (substr ($t, 0, $len_prefix) == $table_prefix) $tables[] = $t;
	 }

	 // backup per table
	 foreach ($tables as $key => $val)
	 {
		$cur_table = $val; $ep = $ed = array ();
		echo '[',$cur_table,']';
		...
		while ($row = mysqli_fetch_row ($res))
		{
			...
			foreach ($row as $k => $v) $cmd .= "'".addslashes($v)."', ";
			$cmd = substr ($cmd, 0, -2);
			$counter += output ($cmd.");\n");
