...
while ($row = sql_fetch_array ($res, 1, 1)) $cfg[$row['config_id']] = $row['config_value'];
...
while ($row = sql_fetch_array ($res)) $lang_list[$row['lang_id']] = $row['lang_value'];
...
{
	if ($file != "." && $file != ".." && substr ($file, 0, 1) != '_' && is_dir ("../skins/$file")) $c_skin["skins/$file"] = $file;
}
closedir($handle);
...
if (empty ($cfg['watermark_file']))
	{ $cfg['watermark'] = ''; $isWatermark = false; }
else
	{ $cfg['watermark'] = "<img src=\"./../public/image/$cfg[watermark_file]\" alt=\"watermark\" />"; $isWatermark = true; }
