...
switch ($mode)
{
	case 'restore':
		...
		if (empty ($row) || empty ($row['log_previous'])) admin_die (sprintf ($lang['msg']['echo'], 'Log not available!'));
		$old = (unserialize (gzuncompress (base64_decode ($row['log_previous']))));
		if (empty ($old)) admin_die (sprintf ($lang['msg']['echo'], 'Previous values not available!'));
		if (($row['log_action'] == 1) || ($row['log_action'] == 4) || ($row['log_action'] == 5)) admin_die (sprintf ($lang['msg']['echo'], 'Unfortunately qEngine can\'t restore changed files or item creation.'));

		// create sql
		$sql = array ();
		foreach ($old as $k => $v) if (!is_numeric ($k)) $sql[] = "$k='".addslashes($v)."'";
		...
		if (!empty ($sql))
		{
			...
			if (!$tbl || empty ($tbl['Column_name'])) admin_die (sprintf ($lang['msg']['echo'], 'This feature is not supported by this version of MySQL!'));
	...
	case 'detail':
		...
		if (empty ($row)) redir ($config['site_url'].'/'.$config['admin_folder'].'/qadmin_log.php');
