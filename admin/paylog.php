...
while ($row = sql_fetch_array ($res)) $payment_def[$row['mod_id']] = $row['mod_name'];
...
switch ($mode)
{
	...
	case 'detail':
		...
		if (empty ($row)) admin_die ('Log not found! Payment method may not support payment log.');

		// outuput
		if (empty ($row['order_id'])) $row['order_id'] = "Missing ID";
		if (!empty ($row['notes']))
		{
			$doh = array ();
			$foo = explode ("\r\n", $row['notes']);
			foreach ($foo as $val) $array[] = "<li>$val</li>";
