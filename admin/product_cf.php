...
function post_func ($cmd, $id, $savenew, $old, $new)
{
	...
	if ($row['cf_category'] == ',,') sql_query ("UPDATE ".$db_prefix."product_cf_define SET cf_category='' WHERE idx='$id' LIMIT 1");
	...
	if ($old['is_removed'] || $new['is_removed'])
	{
		...
		if (!empty ($foo)) sql_query ("ALTER TABLE `".$db_prefix."product_cf_value` DROP `cf_".$id."`");
	...
	$row['cf_title'] = addslashes ($row['cf_title']);
	$row['cf_help'] = addslashes ($row['cf_help']);
	if (!empty ($row['cf_help'])) $cf_notes_str = $row['cf_title'].' - '.$row['cf_help']; else $cf_notes_str = $row['cf_title'];
...
if (empty ($id)) $id = post_param ('primary_val');
