...
if (empty ($cmd)) $cmd = get_param ('cmd');
...
switch ($cmd)
{
	...
	case 'distro_list':
		...
		foreach ($ce_cache['distro'] as $k=>$r)
		{
			...
			while ($row2 = sql_fetch_array ($res2)) $row['product'] .= $row2['title'].', ';
	...
	case 'do':
		if (empty ($_FILES['csv_file']['tmp_name'])) admin_die ('echo', 'Please supply a CSV file!');
		if (!is_uploaded_file ($_FILES['csv_file']['tmp_name'])) admin_die ('echo', 'Please supply a CSV file!');
		...
		if ($handle)
		{
			...
			{
				$buffer = fgets($handle);
				if (strlen ($buffer > 4096)) admin_die ('echo', 'Each line must be less than 4,096 characters!');
			}
			fclose($handle);
		}
		else
		...
		if ($handle)
		{
			...
			while (($data = fgetcsv($handle, 4096, ",")) !== FALSE)
			{
				...
				if (empty ($data[2])) $ok = false;	// title
				if ($title && ($row == 1)) $ok = false;	// first line is title
				foreach ($data as $val) $val = addslashes ($val);
				...
				{
					if (!$title || ($row > 1)) echo "<br />Row #$row contains invalid information! Please ignore if it's a title row.<br />";
				}
			}
			fclose($handle);
		}
