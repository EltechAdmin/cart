...
$tab = get_param ('tab');
if (empty ($tab)) $tab = post_param ('tab', 0);

$cmd = get_param ('cmd');
if (empty ($cmd)) $cmd = post_param ('cmd');

$lang_id = get_param ('lang_id');
if (empty ($lang_id)) $lang_id = post_param ('lang_id', 'en');
...
if (($tab == 0) && !$cmd) $cmd = 'properties';
elseif (($tab == 6) && !$cmd) $cmd = 'custom';

switch ($cmd)
{
	case 'import':
		if ($config['demo_mode']) admin_die ('demo_mode');
		...
		while ($row = sql_fetch_array ($res)) $lang_list[] = $row['lang_id'];

		$overwrite = post_param ('overwrite');
		if (!is_uploaded_file ($_FILES['xml_file']['tmp_name'])) admin_die ('admin_err');

		$xml = read_xml ($_FILES['xml_file']['tmp_name']);
		$new_lang_id = $xml['qmodule']['#']['id']['0']['#'];
		if ($xml['qmodule']['@']['type'] != 'langpack') admin_die ('<h1>Failed</h1><p>Not a language pack!</p>');
		if ($xml['qmodule']['@']['version'] != '1.0.0') admin_die ('<h1>Failed</h1><p>Incompatible version!</p>');
		if (in_array ($new_lang_id, $lang_list) && !$overwrite) admin_die ('<h1>Failed</h1><p>Language has been existed in the database. Operation aborted.</p>');

		// remove old lang if necessary
		if ($overwrite) sql_query ("DELETE FROM ".$db_prefix."language WHERE lang_id='$new_lang_id'");

		$keys = $xml['qmodule']['#']['language']['0']['#']['lang_key'];
		$vals = $xml['qmodule']['#']['language']['0']['#']['lang_value'];
		if (count ($vals) != count ($keys)) admin_die ('Number of language keys &amp; language values are not the same. File may be corrupted?');
		foreach ($keys as $k => $v)
		{
			$kk = $v['#'];
	...
	case 'export':
		...
		while ($row = sql_fetch_array ($res)) $langcfg[$row['lang_key']] = $row['lang_value'];
		...
		{
			if ($row['lang_key'] == '_config:cache') $row['lang_value'] = '';
	...
	case 'del_lang':
		...
		AXSRF_check ();
		if ($lang_id == 'en') admin_die ('<h1>Failed!</h1><p>You can not remove default [English] language!</p>');
	...
	case 'copy_lang':
		...
		if (empty ($new_lang_id) || empty ($new_lang_name)) admin_die ($lang['msg']['admin_err']);
		$foo = sql_qquery ("SELECT * FROM ".$db_prefix."language WHERE lang_id='$new_lang_id' LIMIT 1");
		if (!empty ($foo)) admin_die ('admin_err');
	...
	case 'save_custom':
		...
		foreach ($_POST as $k => $v)
		{
			...
			if (substr ($k, 0, 3) == 'ck_')
			{
				...
				if (!empty ($vv))
				{
					if (empty ($kk)) $kk = strtolower (preg_replace ("/[^a-zA-Z0-9]/", "_", $vv));
					if (substr ($kk, 0, 2) != 'l_') $kk = 'l_'.$kk;
					$new[$kk] = $vv;
					$foo = sql_qquery ("SELECT * FROM ".$db_prefix."language WHERE lang_id='$lang_id' AND lang_key='$kk' LIMIT 1");
					if (empty ($foo))
					{
						foreach ($lang_list as $k => $v) sql_query ("INSERT INTO ".$db_prefix."language SET lang_id='$v', lang_key='$kk', lang_value='$vv'");
					}
				}
			}
		}
	...
	case 'save_properties':
		...
		if (!empty ($lang_name)) sql_query ("UPDATE ".$db_prefix."language SET lang_value='$lang_name' WHERE lang_id='$lang_id' AND lang_key='_config:lang_name' LIMIT 1");
	...
	case 'save':
		...
		foreach ($langkeys[$k] as $k2 => $v2)
		{
			...
			$v2 = str_replace ('|', '.', $v2);
			if (!empty ($vv)) sql_query ("UPDATE ".$db_prefix."language SET lang_value='$vv' WHERE lang_id='$lang_id' AND lang_key='$v2' LIMIT 1");
		}
		sql_query ("UPDATE ".$db_prefix."language SET lang_value='' WHERE lang_id='$lang_id' AND lang_key='_config:cache' LIMIT 1");
		admin_die ('admin_ok');
	break;
	...
	case 'custom':
		...
		while ($row = sql_fetch_array ($res))
		{
			if (substr ($row['lang_key'], 0, 8) != '_config:') { $langrow[$row['lang_key']] = $row['lang_value']; $langkey[] = $row['lang_key']; }
			if ($row['lang_key'] == '_config:lang_name') $lang_name = $row['lang_value'];
		}
	...
	case 'properties':
		...
		while ($row = sql_fetch_array ($res)) $txt['lang_list'] .= "<li><a href=\"lang.php?tab=0&amp;lang_id=$row[lang_id]\">$row[lang_value]</a></li>";
		...
		while ($row = sql_fetch_array ($res)) $txt['cfg_'.$row['lang_key']] = $row['lang_value'];
	...
	default:
		...
		while ($row = sql_fetch_array ($res))
		{
			if (substr ($row['lang_key'], 0, 8) != '_config:') $langedit[$row['lang_key']] = $row['lang_value'];
			if ($row['lang_key'] == '_config:lang_name') $lang_name = $row['lang_value'];
		}
		...
		foreach ($langkeys[$k] as $k2 => $v2)
		{
			...
			// create the form
			$v2 = str_replace ('.', '|', $v2);
			if ($k == 'msg') $row['lang_val'] = "<textarea name=\"$v2\" style=\"width:500px;height:50px\">$val</textarea>";
			elseif ($k == 'mail') $row['lang_val'] = "<textarea name=\"$v2\" style=\"width:500px;height:100px\">$val</textarea>";
			else $row['lang_val'] = "<input type=\"text\" name=\"$v2\" value=\"$val\" style=\"width:500px\" id=\"textbox_$k2\"/> <a href=\"javascript:change($k2)\" id=\"change_$k2\"><span class=\"glyphicon glyphicon-resize-full\" title=\"expand\"></span></a>";
			$txt['block_list'] .= quick_tpl ($tpl_block['list'], $row);
		}
		$txt['tab'] = $tab;
		$txt['lang_id'] = $lang_id;
