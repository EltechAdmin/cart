...
function kickme ($log = true)
{
	...
	if ($log) sql_query ("INSERT INTO ".$db_prefix."ip_log VALUES ('', UNIX_TIMESTAMP(), '$ip', '$admin_id', 'A', '0')");
	qvc_init ();
...
if (!$level) kickme ();
if (($row['user_id'] != $admin_id) || ($row['user_passwd'] != $admin_passwd)) kickme ();
...
if (!strpos ('.'.$redir, $config['site_url'])) $redir = $config['site_url'].'/'.$config['admin_folder'].'/index.php';
redir ($redir);
kickme ();
?>