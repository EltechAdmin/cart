...
function post_func ($cmd, $id, $savenew = false)
{
	...
	// when removed -> done
	if ($cmd == 'remove_item') redir ($config['site_url'].'/'.$config['admin_folder'].'/coupon.php');
...
{
	$inf = sql_qquery ("SELECT * FROM ".$db_prefix."gift WHERE idx='$id' LIMIT 1");
	if ($inf['redeem_user_id']) $redeem = true;
	if ($inf['redeem_sent']) $email_sent = true;
	if ($inf['redeem_date']) $redeem_date = convert_date (date ('Y-m-d', $inf['redeem_date'])).' @ '.date ('H:i', $inf['redeem_date']);
}
