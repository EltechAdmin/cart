...
if (!$current_user_info['admin_level']) redir ($config['site_url'].'/'.$config['admin_folder'].'/login.php');
...
switch ($cmd)
{
	case 'feed':
		...
		if (!$xml) die ('<b>NOTICE</b> Can not open RSS feed, please open C97net web site for latest update!');
	...
	case 'notify_go':
		...
		if (!empty ($row))
		{
			...
			if (!empty ($row['notify_url'])) redir ($row['notify_url']);
		}
...
{
	$i++;
	if (substr ($row['user_id'], 0, 6) == 'guest*') $row['user_id'] = '[ Xpress Checkout ]';
	$row['n'] = $i;
...
if (empty ($ss1)) $ss1 = array ('idx' => 0, 'log_time' => 0, 'log_success' => 0, 'log_user_id' => '(never)', 'log_ip_addr' => '0.0.0.0');
