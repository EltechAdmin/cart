...
while ($row = sql_fetch_array ($res)) ${$row['config_id']} = post_param ($row['config_id'], '', 'html');

// escape some html
$cat_separator = post_param ('cat_separator', '', 'html');
if (empty ($cat_separator)) $cat_separator = '&bull;';

// currency flags
for ($i = 0; $i <= 5; $i++) if (!empty ($_FILES['curr_flag_'.$i]['tmp_name']))
{
