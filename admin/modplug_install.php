...
function generate_admin_menu ($what)
{
	...
	{
		if ($what == 'module') $id = $row['mod_id']; else $id = $row['plug_id'];
...
$cmd = get_param ('cmd');
$id = get_param ('id');
if (empty ($id)) $id = get_param ('mod_id');
...
switch ($cmd)
{
	...
	case 'uninstall':
	 ...
	 if (empty ($row)) admin_die ($lang['msg']['mod_not_installed']);
		...
		if ($remove_file)
		{
			...
			while ($ok)
			{
				if (!empty ($xml['qmodule']['#']['uninstall'][0]['#']['folder'][$i]['#']))
				{
					...
					foreach ($fl as $val) @unlink ($fold.'/'.$val);
					@rmdir ($fold);
					$i++;
				}
				else
	...
	case 'install':
	 ...
	 // see if it installed
	 $row = sql_qquery ("SELECT * FROM ".$db_prefix."module WHERE mod_id = '$id' LIMIT 1");
	 if (!empty ($row)) admin_die ('mod_installed');
