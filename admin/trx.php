...
if (empty ($summary)) { if ($cmd == 'print') just_die ('Error!', 'Order ID not found!'); else admin_die ('echo', 'Order ID not found!', $config['site_url'].'/'.$config['admin_folder'].'/trx_list.php'); };
...
if (empty ($sum)) admin_die ('echo', 'Order not found!');
if ($sum['order_status'] == 'X') $order_dis = 1; else $order_dis = 0;
if ($sum['order_paystat'] == 'X') $pay_dis = 1; else $pay_dis = 0;
if (empty ($sum['order_notes'])) $txt['order_notes'] = 'n/a'; else $txt['order_notes'] = $sum['order_notes'];
...
// output
if (substr ($txt['user_id'], 0, 6) == 'guest*') $txt['user_id'] = '[ Xpress Checkout ]';
$txt['order_id'] = $order_id;
$txt['main_body'] = quick_tpl ($tpl, $txt);
if ($cmd == 'print') flush_tpl ('adm_popup'); else flush_tpl ('adm');
?>