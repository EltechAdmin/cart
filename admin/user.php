...
if (empty ($qadmin_cmd)) $qadmin_cmd = post_param ('qadmin_cmd');

// rights view
$lvl = admin_check (4);

// you can't defined higher level!
if ($lvl == 4) { unset ($admin_level_def[5]); unset ($user_level_def[5]); }

// you can't edit/view higher level!
if (!empty ($id))
{
	$foo = sql_qquery ("SELECT admin_level FROM ".$db_prefix."user WHERE user_id='$id' LIMIT 1");
	if ($foo['admin_level'] > $lvl) admin_die ($lang['msg']['no_level']);
}
...
if ($qadmin_cmd == 'new') $qadmin_def['user_level']['value'] = 1; else $qadmin_def['user_level']['value'] = 'sql';
