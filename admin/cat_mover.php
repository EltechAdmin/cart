...
{
	case 'save':
		if (empty ($target_cat)) admin_die ('Please select a target category.');
		if (!array_key_exists ($target_cat, $ce_cache['cat_name_def'])) admin_die ('Please select a target category.');
		$items = checkbox_param ('items', 'get', true);
		foreach ($items as $k => $v) sql_query ("UPDATE ".$db_prefix."products SET cat_id='$target_cat' WHERE idx='$v' LIMIT 1");
		admin_die ('admin_ok');
	break;


	default:
		if (empty ($ce_cache['cat_structure'])) admin_die ('No categories defined. Please create one first!');
