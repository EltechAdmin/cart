...
function do_custom_sql ($item_id)
{
	...
	if (!$old_val) sql_query ("INSERT INTO ".$db_prefix."product_cf_value SET item_id='$item_id'");
		...
		switch ($row['cf_type'])
		{
			...
			case 'img':
				if (!empty ($_FILES[$key]['name'])  && (!$config['demo_mode']))
				{
					...
					if (!empty ($config['watermark_file'])) image_watermark ("$ifolder/".$_FILES[$key]['name'], './../public/image/'.$config['watermark_file']);
			...
			case 'private':
				if (!empty ($_FILES[$key]['name']) && (!$config['demo_mode']))
				{
					...
					if ($s['success']) $val = $s[0]['filename']; else $val = $old;
				}
				else
					$val = $old;
			break;
	...
	$sql = implode (', ', $output);
	if ($sql) sql_query ("UPDATE ".$db_prefix."product_cf_value SET $sql WHERE item_id='$item_id' LIMIT 1");
}
...
// multiple cats
if (!empty ($add_category)) $add_category = ','.$add_category.',';

// qty price
$ppp = array ();
foreach ($_POST as $k => $v) if (substr ($k, 0, 10) == 'price_qty_')
{
	$i = substr ($k, 11);
	$qpq = post_param ('price_qty_q'.$i);
	$qpp = post_param ('price_qty_p'.$i);
	if (!empty ($qpq)) $ppp[$qpq] = $qpp;
}
ksort ($ppp);
...
if (!$item_id) $mode = 'new'; else $mode = 'edit';
if ($del_item) $mode = 'del_item';
...
// invisible?
if (!$is_invisible) $is_invisible='0';
...
if ($_FILES['digital_file']['name'])
{
	...
	if (!$f['success']) admin_die ($lang['msg']['can_not_upload']);
...
if ($_FILES['image']['name'])
{
	...
	while (!$ok)
	{
		$i++;
		$fn = $item_id.'_'.$i;
		if (!file_exists ("$folder/$fn.jpg")) $ok = TRUE;
	}
	...
	if ($config['optimizer'])
	{
		...
		if (!empty ($config['watermark_file'])) image_watermark ($target, './../public/image/'.$config['watermark_file']);
		if (!file_exists ($target)) admin_die ($lang['msg']['can_not_upload']);
		@chmod ($target, 0644);
	}
	else
	{
		if (!$config['demo_mode'])
		{
			if (!@upload_file ('image', $target)) admin_die ($lang['msg']['can_not_upload']);
			if (!empty ($config['watermark_file'])) image_watermark ($target, './../public/image/'.$config['watermark_file']);
			@chmod ($target, 0644);
		}
	}
...
if ($copy_item)
{
	...
	if ($copy_img)
	{
		...
		while (!$ok)
		{
			$i++;
			$fn =
			$sf = $sfolder.'/'.$item_id.'_'.$i.'.jpg'; $st = $tfolder.'/'.$item_id.'_'.$i.'.jpg';
			$tf = $sfolder.'/'.$new_item.'_'.$i.'.jpg'; $tt = $tfolder.'/'.$new_item.'_'.$i.'.jpg';
			if (file_exists ($sf))
			{
				copy ($sf, $tf);
				copy ($st, $tt);
	...
	// redir?
	if ($copy_switch) admin_die ('Product has been copied succesfully. You are now editing <b>the copied</b> product.', $config['site_url'].'/'.$config['admin_folder'].'/product.php?cmd=edit&item_id='.$new_item);
}
