...
switch ($cmd)
{
	case 'save':
		...
		{
			$foo = array ();
			for ($i = 0; $i <= 5; $i++)	$foo[$i] = post_param ($row['config_id'].'_'.$i);
			$s = serialize ($foo);
	...
	default:
		...
		while ($row = sql_fetch_array ($res))
		{
			...
			{
				for ($i = 1; $i <= 5; $i++)	$row['check'.$i] = create_tickbox_form ($row['config_id'].'_'.$i, '', $val[$i]);
			...
			{
				for ($i = 0; $i <= 5; $i++)	$row['check'.$i] = create_tickbox_form ($row['config_id'].'_'.$i, '', $val[$i]);
