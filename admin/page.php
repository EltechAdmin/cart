...
function post_func ($cmd, $id, $savenew = false)
{
	...
	if ($cmd == 'remove_item') admin_die ('admin_ok', $config['site_url'].'/'.$config['admin_folder'].'/page.php');
	if ($savenew)
...
$gid = get_param ('gid');
$id = get_param ('id');
if (empty ($id)) $id = get_param ('primary_val');
if (empty ($gid)) $gid = post_param ('group_id');
...
{
	if (substr ($v, 0, 5) == 'body_') $group_template_def[$v] = $v;
	if ((substr ($v, 0, 5) == 'page_') && ($v != 'page_list.tpl')) $page_template_def[$v] = $v;
}
...
if ($id)
{
	...
	$mm = array (); $i = 0;
	if ($page_info['page_related']) foreach ($foo as $k => $v)
	{
		$i++;
...
// determine mode
if (empty ($gid) && empty ($gid_sql)) $mode = 'new';
elseif (!empty ($gid) && empty ($gid_sql)) $mode = 'new_change';
elseif (empty ($gid) && !empty ($gid_sql)) { $mode = 'edit'; $gid = $gid_sql; }
elseif (!empty ($gid) && !empty ($gid_sql)) $mode = 'edit_change';
else $mode = 'new';
...
while ($row = sql_fetch_array ($res)) $group_def[$row['group_id']] = $row['group_title'];
...
while ($row = sql_fetch_array ($res)) $cat_def[$row['idx']] = $row['cat_name'];
...
if ($gid) $qadmin_def['group_id']['value'] = $gid;
...
if ($mode == 'new') { $qadmin_def['page_title']['type'] = 'disabled'; $qadmin_def['page_title']['value'] = 'Please choose a group first.'; }
...
if ($id) $qadmin_def['page_body']['help'] = 'Need a custom page? Easy, create your page in HTML and save as page_'.$id.'.tpl, and put it in /skins/_common folder.';
...
if ($id) $qadmin_def['last_update']['value'] = convert_date (date ('Y-m-d', $page_info['last_update'])).' @ '.date ('h:ia', $page_info['last_update']);
...
if ($filter_by == '') $_GET['filter_by'] = 1;

// - filter sql
$fs = 'cat_id != 0,';
$ft = 'All Contents,';
foreach ($group_def as $k => $v) { $fs .= "group_id='$k',"; $ft .= "$v,"; }; $fs = substr ($fs, 0, -1); $ft = substr ($ft, 0, -1);
