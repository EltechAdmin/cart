...
function get_tree ($menu_id, $id, $level = 0, $max_depth = 1)
{
	...
	if ($config['menu_class'] == 'bsnav') $bsnav = true; else $bsnav = false;
	...
	while ($row = sql_fetch_array ($res))
	{
		...
		if (!$exists)
		{
			if ($level == 0)
			{
				if ($bsnav) $menu_class = 'nav navbar-nav'; else $menu_class= $config['menu_class'];
		...
		if (!empty ($ccc))
		{
			...
			if ($bsnav && ($level < $max_depth))	$current = str_repeat ("\t", $level + 1)."<li class=\"dropdown\"><a href=\"#\" class=\"dropdown-toggle\" data-toggle=\"dropdown\" role=\"button\">$row[menu_item] <span class=\"caret\"></span></a>";
	...
	if ($exists) { $menu_cache .= str_repeat ("\t", $level)."</ul>\n"; }
...
if (!empty ($midx))
{
	...
	if (!$cfg) admin_die (sprintf ($lang['msg']['echo'], 'Invalid Menu IDX!'));
	if ($cfg['menu_preset'] != '--') $config['menu_class'] = $cfg['menu_preset']; else $config['menu_class'] = $cfg['menu_class'];
...
switch ($cmd)
{
	...
	case 'del_menu':
		...
		if ($foo['menu_locked']) admin_die ($lang['msg']['menuman_locked_err']);
	...
	case 'dn':
		...
		// same parent?
		foreach ($item_list as $k => $v) if ($v['idx'] == $iidx) { $c = $k; break; }
		$n = $c + 1;
		$norder = $item_list[$c]['order'] + 5;

		// top most?
		if ($c >= count ($item_list)) redir ();
		...
		else
		{
			...
			$i = $c; $cl = $item_list[$c]['level']; $ok = false;
			while (!$ok)
			{
				$i++;
				if (!empty ($item_list[$i]['idx']))
				{
					if ($item_list[$i]['level'] == $cl) $ok = true;
				}
				else
				{
					$ok = true;
	...
	case 'design_refresh':
		$tpl_mode = 'design';
		$tpl = load_tpl ('adm', 'menu_man.tpl');
		if (empty ($midx)) admin_die (sprintf ($lang['msg']['echo'], 'Invalid Menu IDX!'));
		...
		foreach ($item_list as $k => $v)
		{
			...
			{
				$any = false;
				for ($i = $k - 1; $i > 0; $i--) if (($item_list[$i]['parent'] == $v['parent']) && ($item_list[$i]['level'] == $v['level'])) $any = true;
				if (!$any) $v['up'] = $upd; else $v['up'] = sprintf ($up, $midx, $v['idx']);
			}
			...
			{
				$any = false;
				for ($i = $k + 1; $i <= $max; $i++) if (($item_list[$i]['parent'] == $v['parent']) && ($item_list[$i]['level'] == $v['level'])) $any = true;
				if (!$any) $v['dn'] = $dnd; else $v['dn'] = sprintf ($dn, $midx, $v['idx']);
			}
