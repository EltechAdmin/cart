...
function get_cf ($cat_id, $cf_val)
{
	...
	foreach ($ce_cache['cf_define'] as $row)
	{
		...
		foreach ($foo as $k => $v) if (($v == $cat_id) || (!$row['cf_category']) || ($row['cf_category'] == ',,')) $cf_define[] = 'cf_'.$row['idx'];
	}

	foreach ($cf_define as $cat_cf)
	{
		$row = $ce_cache['cf_define'][$cat_cf];
		if ($row['cf_help']) $row['cf_help'] = '<span class="glyphicon glyphicon-info-sign help tips" title="'.$row['cf_help'].'"></span>';
		$key = 'cf_'.$row['idx'];
		...
		switch ($row['cf_type'])
		{
			...
			case 'video':
				...
				if ($val) $field .= "<a href=\"$val\" target=\"_blank\"><span class=\"glyphicon glyphicon-film\"></span></a>";
			break;
...
switch ($cmd)
{
	case 'del_digital':
		...
		if (empty ($row['idx'])) admin_die ('item_id_not_found');
		if (empty ($row['digital_file'])) admin_die ('echo', 'This item doesn\'t have digital download!');
	...
	case 'edit':
		...
		if (empty ($row['idx'])) admin_die ('<h1>Error!</h1><p>Item Not Found!</p>');
		...
		if ($i == 1) $no_image = true; else $no_image = false;

		// get see also
		$i = 0;
		$foo = explode (',', $row['see_also']);
		$mm = array ();
		if ($row['see_also']) foreach ($foo as $k => $v)
		{
			$i++;
		...
		$i = 0;
		if ($pq) foreach ($pq as $k => $v)
		{ $i++; $row['price_qty_q'.$i] = $k; $row['price_qty_p'.$i] = $v; }
		for ($j = $i + 1; $j <= 5; $j++) { $row['price_qty_q'.$j] = $row['price_qty_p'.$j] = ''; }
		...
		if ($popup) flush_tpl ('adm_popup'); else flush_tpl ('adm');
	break;
	...
	default:
		...
		for ($i = 1; $i <= 8; $i++) $row['price_qty_q'.$i] = $row['price_qty_p'.$i] = '';
