...
if (empty ($q)) flush_json (9998, 'Undefined query (q) for ajax mode: '.$cmd);
...
switch ($cmd)
{
	...
	case 'notifylist':
		...
		while ($row = sql_fetch_array ($res))
		{
			...
			if ($row['notify_read']) $output .= "$ii\n<hr />"; else $output .= "<b>$ii</b>\n<hr />";
		}
		if (!$i) $output .= "You don't have any notifications!"; else $output = substr ($output, 0, -6);
	...
	case 'page':
		...
		while ($row = sql_fetch_array ($res)) $output[] = array ('value' => "$row[page_id]: $row[page_title]", 'data' => $row['page_id']);

		$res = sql_query ("SELECT menu_id, menu_title FROM ".$db_prefix."menu_set WHERE (menu_title LIKE '%$q%') OR (menu_id LIKE '$q%') LIMIT $limit");
		while ($row = sql_fetch_array ($res)) $output[] = array ('value' => "(submenu) $row[menu_title]", 'data' => "[[sm:$row[menu_id]]]");
	...
	case 'related_page':
		...
		while ($row = sql_fetch_array ($res)) $suggest[] = array ('id' => $row['page_id'], 'name' => $row['page_title']);
