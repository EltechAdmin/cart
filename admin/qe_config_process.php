...
while ($row = sql_fetch_array ($res)) {${$row['config_id']} = post_param ($row['config_id']); $old[$row['config_id']] = $row['config_value']; }

// change sef url?
$change_sef = false;
if ($enable_adp != $old['enable_adp']) $change_sef = true;
...
if ($adp_extension[0] == '.') $adp_extension = substr ($adp_extension, 1);
if ($list_ipp < 5) $list_ipp = 5;

// escape some html
$num_currency = html_unentities ($num_currency);

// multi lang
if ($multi_lang) $multi_lang = 0; else $multi_lang = 1;
