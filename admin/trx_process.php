...
if ($order_status == '-') $order_status = $row['order_status'];
if ($order_paystat == '-') $order_paystat = $row['order_paystat'];

//
$user_id = $row['user_id'];
$user = get_user_info ($user_id);
if ($row['order_paystat'] != $order_paystat) $update_paystat = true; else $update_paystat = false;
if ($row['order_status'] != $order_status) $update_status = true; else $update_status = false;
...
if ($update_status)
{
	switch ($order_status)
	{
		...
		case 'D':
			...
			if ($foo['order_shipped'] == '0000-00-00') sql_query ("UPDATE ".$db_prefix."order_summary SET order_shipped = '$sql_now' WHERE order_id = '$order_id' LIMIT 1");
		...
		case 'C':
			...
			if ($foo['order_shipped'] == '0000-00-00') sql_query ("UPDATE ".$db_prefix."order_summary SET order_shipped = '$sql_now' WHERE order_id = '$order_id' LIMIT 1");
			if ($foo['order_delivered'] == '0000-00-00') sql_query ("UPDATE ".$db_prefix."order_summary SET order_delivered = '$sql_now' WHERE order_id = '$order_id' LIMIT 1");
...
if ($update_status && (($order_status == 'S') || ($order_status == 'P') || ($order_status == 'D') || ($order_status == 'C')))
{
	...
	{
		// sold + 1
		if ($order_status == 'S') sql_query ("UPDATE ".$db_prefix."products SET stat_purchased=stat_purchased+$row[qty], stat_last_purchased='$sql_today' WHERE idx='$row[idx]' LIMIT 1");

		// digital
		if (empty ($row['digital_prod'])) $all_digi = FALSE;
	}
