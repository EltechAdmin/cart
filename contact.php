...
$cmd = get_param ('cmd'); if (empty ($cmd)) $cmd = post_param ('cmd');
...
switch ($cmd)
{
	case 'send':
		...
		if (!validate_email_address($email)) $err[] = "$lang[l_email_error]";
		if (empty($name)) $err[] = $lang['l_name_empty'];
		if (empty($body)) $err[] = $lang['l_message_empty'];

		// any error?
		if (!empty($err)) msg_die (sprintf ($lang['msg']['contact_err'], '<ul><li>'.implode ('</li><li>', $err).'</li></ul>'));
