...
switch ($mod_id)
{
	...
	default:
		// simple page search
		if (empty ($query)) msg_die (sprintf ($lang['msg']['echo'], $lang['l_search_no_result']));
		// load tpl
		...
		{
			$i++;
			if ($config['enable_adp'] && $row['permalink']) $row['page_url'] = $row['permalink']; else $row['page_url'] = "page.php?pid=$row[page_id]";
		...
		if (!$i) $no_result = true; else $no_result = false;
